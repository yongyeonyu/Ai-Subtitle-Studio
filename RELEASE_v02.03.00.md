...
This-Update: v02.03.03 리팩토링, NAS 선택 UX, MLX Whisper JSON 안정화, AI 프리셋 연동 기록 추가
Copilot-Handoff: v02.03.03 커밋 기준. 다음 작업 전 git status와 장시간 자동처리 실사용 로그를 먼저 확인하세요.
-->
...
## 2026-04-27 v02.03.03 체크포인트

- `APP_VERSION`을 `02.03.03`으로 승격
- `subtitle_engine.py`에서 SRT 저장/Gemini/Ollama adapter 분리
- `EditorWidget` 대형 mixin을 멀티클립 context, 멀티클립 ops, 화자 ops, 비디오 control로 분리
- `settings_gap.py`의 시뮬레이터 위젯을 `gap_simulator.py`로 분리
- 글로벌 캔버스 seek 시 `QScrollBar.setValue(float)` 크래시 수정
- 자동 배치 에디터 진입 시 `QTimer` 지역 변수 shadowing 크래시 수정
- MLX Whisper 진행률 stdout 혼입으로 JSONL 파싱이 깨지는 문제 수정
- NAS 폴더/파일 선택창에 선택 전파, 자동감지/폴더 전체 선택 분리, 저장 버튼, 제외 폴더 회색 표시 추가
- 자동시작 ON 상태에서 iCloud/NAS 자동처리 카드에 Watchdog 타이머 표시
- AI 설정 메뉴에 오디오 프리셋을 추가하고 AudioAI/VAD/Whisper/LLM 추천값을 동시 적용
