...
처음 잡혀 있는 중분류 초안은 참고용일 뿐이며, 전체 자막을 다시 이해한 뒤 주제 흐름에 맞게 과감하게 합치거나 분리할 수 있다.
최종 목표는 기존 경계를 유지하는 것이 아니라 자막 전체와 영상 흐름을 이해하고 주제별로 자연스럽게 묶인 중분류를 만드는 것이다.
중분류 A/B/C/D는 화면 전환, 주제 전환, 장소 전환, 행동 단계 전환처럼 시청자가 장면이 바뀌었다고 느끼는 큰 흐름 단위다.
실외에서 실내로 들어가거나, 차량/매장/스튜디오처럼 공간의 울림과 배경소음이 분명히 달라지는 음향 환경 전환도 강한 중분류 후보로 본다.
다만 그런 음향 경계로 먼저 나뉜 구간 안에서도 자막 주제가 더 갈라지면 그 안에서 다시 둘 이상으로 분리할 수 있다.
