...
STAGE_ORDER = ["preprocess", "audio", "stt1", "stt2", "vad", "subtitle_llm", "roughcut_llm"]
...
def generation_stage_keys(status_text: object, *, stt_ensemble_enabled: bool = False) -> set[str]:
    ...
    The order matters: `오디오 추출` is preprocessing, while `[음성]` is audio
    enhancement. Keeping this parser shared prevents the status rail and
    sidebar table from disagreeing on the current stage.
    """
...
def generation_stage_label(status_text: object, *, stt_ensemble_enabled: bool = False) -> str:
    keys = generation_stage_keys(status_text, stt_ensemble_enabled=stt_ensemble_enabled)
    if not keys:
        return ""
    if keys == {"stt1", "stt2"}:
        return "STT 1/2"
