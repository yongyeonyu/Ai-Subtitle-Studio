...
- Latest release checkpoint scope:
  - `v04.01.20` - G2 top-level NLE compatibility projection audit, 04.01.20 version/schema bump, and release docs. This is compatibility audit evidence only; it does not switch project load ownership to NLE, persist `_nle_project_state`, remove legacy `editor_state`, change UI/UX, or prove App Store submission readiness.
...
- Latest focused guard set for `v04.01.20`:
  - Top-level NLE compatibility projection audit: `output/manual_verification/latest/nle_top_level_compatibility_projection_v040120_20260629_1018/nle_persistence_cutover_audit.md` -> `status=blocked`, `prep_ready=true`, `top_level_nle_compatibility_projection_passed=true`, `top_level_nle_canonical_projection_complete=false`, compatibility status `compatibility_projection_partial_blocked`, `not_runtime_change=true`, current canonical owner `legacy_editor_state`, default load source `legacy_editor_state`, explicit projection source `top_level_nle_shadow_metadata`, explicit projection caption/gap count `2/0`, default row/caption/gap count `3/2/1`, `gap_coverage_ready=false`, `canonical_load_owner_change_allowed=false`, `disk_format_cutover_allowed=false`, resave rebuilt shadow from legacy rows `true`
  ...
  - three sub-agent reviews converged on keeping this G2 slice audit-only/blocked: top-level `nle` can be explicitly projected as shadow metadata, canonical load owner remains `legacy_editor_state`, gap-row coverage is not complete, and any canonical owner change needs separate rollback boundary plus legacy/direct-SRT/roughcut/render-export proof. Signed `.pkg`, App Store Connect validation/upload/submission, sandbox smoke, and owner metadata completion remain HOLD for G0.
...
- Latest completed action-item slice: `v04.01.20 G2 Top-Level NLE Compatibility Projection Audit`.
- Current G2 NLE evidence snapshot:
  ...
  - compatibility projection state: `compatibility_projection_partial_blocked`, `not_runtime_change=true`, default project load still uses `legacy_editor_state`, explicit top-level `nle` projection differs from the default legacy captions, gap coverage remains incomplete, and canonical load-owner / disk-format cutover remain disallowed.
