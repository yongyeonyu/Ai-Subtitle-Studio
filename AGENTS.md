...
- Latest release checkpoint scope:
  - `v04.01.21` - G2 top-level NLE gap projection coverage audit, 04.01.21 version/schema bump, and release docs. This closes the previous `top_level_nle_projection_gap_coverage_missing` blocker as compatibility-audit evidence only; it does not switch project load ownership to NLE, persist `_nle_project_state`, remove legacy `editor_state`, change UI/UX, or prove App Store submission readiness.
...
- Latest focused guard set for `v04.01.21`:
  - Top-level NLE gap projection coverage audit: `output/manual_verification/latest/nle_top_level_gap_projection_v040121_20260629_1041/nle_persistence_cutover_audit.md` -> `status=blocked`, `prep_ready=true`, `persistence_cutover_ready=false`, `top_level_nle_compatibility_projection_passed=true`, `top_level_nle_canonical_projection_complete=false`, compatibility status `gap_projection_coverage_ready_blocked`, `not_runtime_change=true`, current canonical owner `legacy_editor_state`, default load source `legacy_editor_state`, explicit projection source `top_level_nle_shadow_metadata`, explicit row/caption/gap count `3/2/1`, default row/caption/gap count `3/2/1`, `gap_coverage_ready=true`, `canonical_load_owner_change_allowed=false`, `disk_format_cutover_allowed=false`, resave rebuilt shadow from legacy rows `true`
  - Previous top-level NLE compatibility projection audit: `output/manual_verification/latest/nle_top_level_compatibility_projection_v040120_20260629_1018/nle_persistence_cutover_audit.md` -> `status=blocked`, explicit projection caption/gap count `2/0`, default row/caption/gap count `3/2/1`, and `gap_coverage_ready=false`
  - NLE canonical load-owner review packet: `output/manual_verification/latest/nle_canonical_load_owner_review_packet_v040119_20260629_095907/nle_canonical_load_owner_review_packet.md` -> `status=owner_review_required_blocked`, `not_runtime_change=true`, `canonical_load_owner_unchanged=true`, current canonical owner `legacy_editor_state`, `canonical_load_owner_change_allowed=false`, `disk_format_cutover_allowed=false`, top-level `nle` schema `ai_subtitle_studio.nle_shadow_project.v1`, role `shadow_metadata`, runtime project state persisted `false`, operation roundtrip `11` families all passed, render/export final invalid/non-monotonic/overlap `0/0/0`, global max active `1`
...
- Latest completed action-item slice: `v04.01.21 G2 Top-Level NLE Gap Projection Coverage Audit`.
...
- Other active queue items:
  - `Mac App Store Submission Readiness`; latest blocker refresh is `output/manual_verification/latest/app_store_identity_metadata_blocker_v040115_20260629_0907/app_store_readiness_audit.md` with `submission_target=mac_app_store_pkg`, `local_packaging_ready=true`, `app_store_submission_ready=false`, blocker count `15`, Apple Development identity present, Apple Distribution identity missing, installer identity missing, and owner-input metadata pending `8/8`. Owner approval for App Store packaging/signing/upload/metadata execution exists, but signed `.pkg`, sandbox smoke, App Store Connect validation, upload/submission, and owner metadata values remain incomplete. Developer ID beta `.dmg` remains a separate opt-in track.
