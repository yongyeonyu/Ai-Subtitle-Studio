...
- `doc/ACTION_ITEMS.md` 실행 또는 새 아이디어 발굴 전에 반드시 이 파일과 `doc/waste_action_item.md`를 먼저 확인한다.
...
- 완료된 `doc/ACTION_ITEMS.md` 항목을 다시 실행하라는 요청이 오면 폐기 후보를 새 근거 없이 재구현하지 않는다.
...
- no-UI benchmark preset을 서로 다른 variant로 병렬 실행할 때 artifact timestamp 충돌 가능성을 무시하지 않는다.
  - 이유: `benchmark-preset`이 초 단위 timestamp 폴더를 쓰는 상태에서 case1/case2 one-shot을 같은 초에 병렬 실행하자 같은 artifact 경로가 재사용되어 one-shot 증거 하나가 덮였다.
  - 다음 원칙: accepted proof로 남길 one-shot/repeat benchmark는 variant별로 순차 실행하거나, artifact 경로가 겹치지 않는지 먼저 확인한다. 병렬 실행 결과는 충돌 가능성이 있으면 accepted evidence로 쓰지 않는다.

- case2 current-code one-shot fast rerun을 repeat 없이 새 accepted artifact로 승격하지 않는다.
  - 이유: `.codex_work/benchmarks/subtitle_pipeline_variants/20260602_171442_661504_32528/benchmark_results.json`은 `14.644s`로 매우 좋아 보였지만, 같은 current code의 sequential rerun `.codex_work/benchmarks/subtitle_pipeline_variants/20260602_173222_091532_51036/benchmark_results.json`은 `27.629s`로 바로 slow band로 돌아갔다.
  - 다음 원칙: current code가 old accepted보다 빨라 보이는 fast one-shot이 나와도, case2 accepted target 갱신은 sequential repeat stability가 확인된 뒤에만 한다. diagnostics용 fast artifact와 accepted artifact는 구분한다.

- precision/recheck runtime을 줄이려는 의도로 worker straggler 한도를 더 타이트하게 조여도 자동으로 빨라진다고 가정하지 않는다.
  - 이유: case2 timing preset에서 `stt_word_timestamp_worker_straggler_timeout_sec`, `min_received_ratio`, `max_missing_chunks`를 더 공격적으로 조였더니 quality/timing은 그대로인데 one-shot `32.544s`, repeat mean `24.1475s`로 accepted runtime band보다 오히려 훨씬 느려졌다.
  - 다음 원칙: runtime-only knob는 "더 엄격하면 더 빠르다"라고 추정하지 말고, one-shot과 repeat를 둘 다 확인해 accepted band를 넘지 못하면 즉시 폐기한다. 다음 실험은 다른 overhead owner나 observability 강화를 우선한다.

- case1에서 raw-restore를 직접 건드리기보다 all-singleton digit common split row를 아예 만들지 않는 쪽이 더 안전할 수 있다.
  - 이유: `17.8에서 11.4 점`처럼 final에서 `split_index=0`만 남고 non-zero split이 전부 `raw_restore -> trim_recent_overlap`으로 사라지는 그룹은, split row를 보존하려는 실험보다 애초에 common split을 만들지 않는 benchmark-only guard가 timing/quality를 더 잘 지켰다.
  - 다음 원칙: case1 같은 Apple-primary 경계에서는 broad preserve/raw-source override보다, `all-singleton digit split group` 같은 좁은 source-shape 조건을 먼저 실험한다.
