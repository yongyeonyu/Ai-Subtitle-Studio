...
- `NLE Timeline Canvas State Ownership: Commit-Boundary Mutable Sync`: main timeline canvas read/projection cutover와 Taption center-reorder release sync는 완료됐고, 드래그 중 per-pixel mutable write 없이 남은 body move 또는 기타 release/commit boundary에서만 NLE sync를 확장하는 다음 슬라이스입니다.
