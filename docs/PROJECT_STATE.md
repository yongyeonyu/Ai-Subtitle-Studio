...
- App version: `04.01.04` from `core/runtime/config.py`
- Project schema version: `04.01.04` from `core/project/project_format.py`
- Latest source-app checkpoint: `docs/release_notes/RELEASE_v04.01.04.md`
...
`v04.01.04` is a source-app release checkpoint, not App Store submission proof.
...
- `G3. Realtime NLE STT/VAD Track Visibility And Resource-Balanced Scheduling`: runtime lane owner-map and compact live status/feed slices are complete; continue with scheduler and visual proof only in bounded slices without slowing generation, mixing final surfaces, or changing UI defaults without owner-approved proof.
...
parity, trace-bundle retention, and `v04.01.04` source release proof.
