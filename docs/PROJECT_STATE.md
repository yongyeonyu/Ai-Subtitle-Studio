...
- `NLE Timeline Canvas State Ownership: Commit-Boundary Mutable Sync`: main timeline canvas read/projection cutover, Taption center-reorder release sync, safe pure center-move release sync, complex center gap-absorption/overwrite-trim release sync, timeline inline text commit sync, speaker split text commit sync, and diamond delete commit sync는 완료됐고, 드래그 중 per-pixel mutable write 없이 남은 release/commit source를 audit한 뒤 NLE sync를 확장하는 다음 슬라이스입니다.
