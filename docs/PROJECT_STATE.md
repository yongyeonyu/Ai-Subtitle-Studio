...
- `NLE Timeline Canvas State Ownership: Commit-Boundary Mutable Sync`: main timeline canvas read/projection cutover는 완료됐고, 드래그 중 per-pixel mutable write 없이 release/commit boundary에서만 NLE sync를 확장하는 다음 슬라이스입니다.
