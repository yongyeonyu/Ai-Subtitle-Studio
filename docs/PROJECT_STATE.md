...
- Keep native migration, Swift rewrite, per-pixel NLE writes, App Store packaging/upload, and DMG work opt-in; owner approval exists for the Mac App Store lane, but signed package, validation, and metadata proof are still required.
...
- App version: `04.01.10` from `core/runtime/config.py`
- Project schema version: `04.01.10` from `core/project/project_format.py`
- Latest source-app checkpoint: `docs/release_notes/RELEASE_v04.01.10.md`
...
`v04.01.10` is a source-app release checkpoint, not App Store submission proof.
...
- `G2. Source-App NLE / Taption Editing Continuity`: monitor and preserve the current source-app NLE/Taption editing contracts while keeping persisted NLE disk fields and native migration gated. The close/deferred-save vector-time boundary blocker, final-overlap deferred-save retry guard, and final save/export micro-overlap shared-boundary repair are fixed and archived.
- `G3. Realtime NLE STT/VAD Track Visibility And Resource-Balanced Scheduling`: runtime lane owner-map, compact live status/feed, scheduler-budget telemetry, live runtime observability proof-harness, strong-evidence gate, representative real-media runtime/status proof, final-overlap deferred-save retry guard, and final save/export micro-overlap repair slices are complete; continue with same-media quality/speed/save-reopen/final-export/global-canvas proof only in bounded slices without weakening final authority, slowing generation, mixing final surfaces, or changing UI defaults without owner-approved proof.
...
parity, trace-bundle retention, and `v04.01.10` source release proof.
