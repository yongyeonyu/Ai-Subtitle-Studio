...
- App version: `04.01.27` from `core/runtime/config.py`
- Project schema version: `04.01.27` from `core/project/project_format.py`
- Latest source-app checkpoint: `docs/release_notes/RELEASE_v04.01.27.md`
- Latest source quick QA artifact: `output/manual_verification/latest/qa_suite_quick_v040117_20260629_0929`
- Latest NLE top-level canonical load opt-in audit: `output/manual_verification/latest/nle_canonical_load_opt_in_v040127_20260629_1248/nle_persistence_cutover_audit.md`
...
`v04.01.27` is a G2 top-level `nle` canonical load opt-in proof checkpoint. It
allows top-level `nle` to become the project load source only under explicit
owner-approved policy and only when paired `nle` / `nle_snapshot` rows agree,
while preserving legacy `editor_state` on disk for rollback. It is not
`nle_snapshot` standalone canonical sourcing, persisted `_nle_project_state`,
legacy disk-shape replacement, final NLE cutover, UI/UX change, or App Store
submission proof.
...
preflight guard proof, and `v04.01.27` top-level NLE canonical load opt-in
proof.
