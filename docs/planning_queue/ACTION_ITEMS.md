...
- Completed narrow implementation slice: the preview now supports `ㅓ` handle scenario/material height adjustment, card pin hover/connect feedback, connector hover feedback, right-click connector deletion, manual `자동정렬` button layout, a default one-line time sequence, and fixed 3-row parallel cut-candidate stacks. Archive pointer: `COMPLETED_ACTION_ITEMS.md` -> `v04.01.34 Roughcut 3-Row Connector Interaction Preview Checkpoint`.
- Completed narrow implementation slice: the scenario/material preview now shares a Miro-style PyQt6 2D canvas view with Command/Ctrl-wheel zoom, small top-right percentage indicators, pan behavior, `canvas_grid_positions` snapshot persistence, 1-grid manual card placement, orthogonal straight connectors, line-jump overlays, and magnetic pin routing while keeping connector creation from auto-moving cards. Archive pointer: `COMPLETED_ACTION_ITEMS.md` -> `G4 Roughcut Scenario Canvas Rework Slice`.
- Current connector interaction rule: drawing a connector must create only the line/state and must not move cards automatically. Card layout changes happen only when the owner/user presses `자동정렬` or invokes another explicit layout command.
...
9. Reference-driven card arrangement model
   ...
   - Adopted layout rules:
     ...
     - Connector creation must not trigger automatic card sorting. The Miro-like board rule for this app is: connect first, inspect the line, then press `자동정렬` only when the user wants the card layout to move.
   ...
   - Miro-style connector UX improvement plan:
     - Use Miro connection lines as interaction inspiration only: blue/active dots around a selected object, drag-to-connect from a dot, visible connection preview while dragging, optional line labels, line jumps for intersections, and line-end reattachment. Keep implementation local, PyQt6 2D-only, and do not introduce a web canvas or cloud collaboration model.
     - Show connector ports only when useful: selected card, hovered card, or active routing mode. Default clutter must stay low on dense scenario boards.
     - Routing start: clicking or dragging from a card output pin enters routing mode, colors the source pin with the scenario/card line color, and shows a dimmed preview line that follows the cursor.
     - Routing target: when hovering a valid target input pin, highlight the pin and preview the final route. If the target already has one input, create a visually linked copy candidate and attach to the copy instead of stealing the existing input.
     - Input/output rules: each middle-segment card has one input by default; adding another input creates a copy. Each card and scenario card may have multiple outputs. Scenario-card output uses the scenario card color for the card, connected cards, and lines.
     - Scenario color lineage rule: once a card is connected to a scenario card, downstream outputs from that card inherit the same scenario color by default, so the scenario card, connected cards, and connecting lines stay visually identical unless the owner explicitly changes the line role.
     - Sorting rule: connector creation, connector deletion, and connector reattachment do not auto-sort. `자동정렬` is the only normal command that moves cards according to connector topology.
     - Reattachment rule: selecting a connector end and dragging it to another valid input/output should update the line without moving cards. Invalid drops return the endpoint to its previous target.
     - Insert-between rule: when a card is dragged onto an existing connector, show an insertion ghost. Confirming inserts the card between source and target while preserving the one-input rule and leaving layout movement to `자동정렬`.
     - Line editing: allow line selection, hover highlight, right-click delete, future label/edit controls, and future style cycling only within preview state. No line edit may mutate final subtitle/NLE rows until a scenario commit boundary is explicitly crossed.
     - Routing readability: keep orthogonal/rounded paths, card-avoidance routing, line jumps on crossings, and deterministic z-order. If crossings become dense, offer fit/auto-align/filtered-line views rather than moving cards implicitly.
     - Keyboard/modifier plan: preserve normal click/drag selection; later add modifier-assisted straight/orthogonal route lock and snap override only after tests prove it does not conflict with card drag/drop, multi-select, copy/paste, or text editing.
     - Evidence requirements: focused tests must cover connect-without-sort, button-only sort, one-input auto-copy, scenario-card multi-output color sync, connector reattach, insert-between ghost, right-click delete, line jumps, and no hidden NLE/editor mutation before commit.
...
```yaml
app_version: "04.01.35"
document_version: "04.01.35-source-app"
phase: "SOURCE_APP_CONTINUATION_V4_1_0"
