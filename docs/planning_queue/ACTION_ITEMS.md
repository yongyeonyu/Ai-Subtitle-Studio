...
- Latest audit artifact: `output/manual_verification/latest/app_store_current_version_refresh_v040131_20260629_151653/app_store_readiness_audit.md`.
- Latest metadata owner-input package: `output/manual_verification/latest/app_store_metadata_owner_input_package_v040131_20260629_151653/app_store_metadata_owner_input_package.md`.
...
Status: active quality/save-export gate. The representative real-media live runtime observability proof, same-media quality/speed proof, direct save/reopen/export proof, open/start/cancel/close/quit responsiveness proof, active global-canvas responsiveness proof, and active-worker export final-surface regression guard are complete. G3 remains open only for any stronger live active-final proof selected by the queue, such as active pre-final samples where `nle_runtime_track_counts.final > 0` are paired with same-time snapshots and save/export authority checks. This is a runtime/session visualization and scheduling plan only; it does not approve persisted NLE disk-format cutover, STT2 skipping, model downsizing, quality-gate loosening, UI/UX redesign, or production default cache promotion.
