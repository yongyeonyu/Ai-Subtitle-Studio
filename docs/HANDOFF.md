...
- Next NLE action is the remaining commit-boundary mutable timeline sync. Do not write NLE mutable state on every drag pixel; only reconcile the next safe normal body move or other release/commit source after proving Taption drag, magnet, gap, and no-overlap behavior again.
