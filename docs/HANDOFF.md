...
- Completed the G2 legacy disk-shape replacement opt-in proof slice.
- Bumped source-app version and project schema from `04.01.29` to `04.01.30`.
- Extended `core/project/nle_persistence_guard.py`, `core/project/nle_snapshot.py`, `core/project/project_io.py`, and `core/project/project_format.py`.
- Extended `tools/audit_nle_persistence_cutover.py` and focused NLE persistence tests.
- Generated refreshed NLE persistence cutover evidence for the current app version.

### Result

- Current code version: `APP_VERSION=04.01.30`.
- Current project schema version: `PROJECT_SCHEMA_VERSION=04.01.30`.
- NLE audit: `output/manual_verification/latest/nle_legacy_disk_shape_replacement_v040130_20260629_143522/nle_persistence_cutover_audit.md`.
- Current G2 state: `status=blocked`, `prep_ready=true`, `persistence_cutover_ready=false`, overall stoplight `red`, ready/blocked gates `11/1`.
- `legacy_disk_shape_replacement_allowed` is now ready only for explicit owner-approved legacy-compatible `editor_state` row projection from the standalone `nle_snapshot` canonical source.
- Loaded/runtime/reloaded/storage snapshot/runtime/editor_state first caption text remains `legacy replacement canonical first`; cache-hit read/resave hydrates runtime state; forged replacement policy is blocked; Direct SRT precedence is preserved.
- Remaining blocked gate: `final_cutover_ready`.
- This is an explicit legacy-compatible row projection proof only. It is not default authority for all projects, legacy `editor_state` key removal, roughcut sidecar change, final NLE disk-format cutover, UI/UX change, or App Store submission proof.
...
- Focused NLE guard: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_project_nle_persistence_guard.py -k "legacy_disk_shape or runtime_nle_project_state"` -> `5 passed, 17 deselected`.
- Audit guard: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_nle_persistence_cutover_audit.py -k "cutover"` -> `6 passed`.
...
- Audit generation: `QT_QPA_PLATFORM=offscreen ./venv/bin/python tools/audit_nle_persistence_cutover.py --output-dir output/manual_verification/latest/nle_legacy_disk_shape_replacement_v040130_20260629_143522` -> `status=blocked`, ready/blocked gates `11/1`, blockers `1`.
- Project/status guard: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_project_context.py tests/test_cp03_cp04_status_ui.py -k "schema or version or project_file_roundtrip or status"` -> `66 passed, 80 deselected`.
...
- G2 full NLE disk-format cutover remains blocked until `final_cutover_ready` has explicit owner-approved proof.
- G0 has owner approval to proceed, but remains blocked on Apple Distribution and 3rd Party Mac Developer Installer identities, signed App Store `.pkg`, strict content-bound `codesign`, content-bound `pkgutil --check-signature`, sandbox smoke, content-bound App Store Connect validation, upload/submission proof, and owner-approved metadata values JSON.
- G1 collect-cache/default promotion remains owner-review gated; any future default change must be one cache at a time with a rollback commit boundary and focused same-fixture proof.
...
- If continuing G2, treat any final cutover wording or default-authority switch as a separate owner-approved proof gate.
- If continuing G0, collect owner-approved metadata values JSON or configure Apple Distribution / installer identities before package/upload execution.

## Previous Handoff - 2026-06-29 v04.01.29 / G2 Runtime _nle_project_state Persistence Opt-In Proof

### Scope

- Completed the G2 runtime `_nle_project_state` persistence opt-in proof slice.
- Bumped source-app version and project schema from `04.01.28` to `04.01.29`.
- Extended `core/project/nle_persistence_guard.py`, `core/project/nle_project_state.py`, `core/project/project_io.py`, and `core/project/project_format.py`.
- Extended `tools/audit_nle_persistence_cutover.py` and focused NLE persistence tests.
- Generated refreshed NLE persistence cutover evidence for the current app version.

### Result

- Current code version: `APP_VERSION=04.01.29`.
- Current project schema version: `PROJECT_SCHEMA_VERSION=04.01.29`.
- NLE audit: `output/manual_verification/latest/nle_runtime_state_persistence_v040129_20260629_140053/nle_persistence_cutover_audit.md`.
- Current G2 state: `status=blocked`, `prep_ready=true`, `persistence_cutover_ready=false`, overall stoplight `red`, ready/blocked gates `10/2`.
- `runtime_project_state_persistence_allowed` is now ready only for explicitly owner-approved supplemental `_nle_project_state` payloads tied to standalone `nle_snapshot` canonical load-source policy.
- Loaded/runtime/reloaded/storage snapshot/runtime first caption text remains `runtime persisted snapshot first`; cache-hit read/resave hydrates runtime state; legacy `editor_state` first caption text after resave remains `first`.
- Remaining blocked gates: `legacy_disk_shape_replacement_allowed` and `final_cutover_ready`.
- This is an explicit supplemental runtime-state persistence proof only. It is not default project authority, legacy `editor_state` replacement, Direct SRT precedence change, roughcut sidecar change, final NLE disk-format cutover, UI/UX change, or App Store submission proof.
...
- Audit generation: `QT_QPA_PLATFORM=offscreen ./venv/bin/python tools/audit_nle_persistence_cutover.py --output-dir output/manual_verification/latest/nle_runtime_state_persistence_v040129_20260629_140053` -> `status=blocked`, ready/blocked gates `10/2`, blockers `2`.
- Project/status guard: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_project_context.py tests/test_cp03_cp04_status_ui.py -k "schema or version or project_file_roundtrip or status"` -> `66 passed, 80 deselected`.
- Direct SRT focused guard: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_project_segment_reload.py -k "direct_srt_rows_to_runtime_nle_state or direct_srt_readback_drift_without_overwriting_srt_rows" tests/test_editor_autosave_cleanup.py -k "direct_srt_mode or direct_srt_micro_overlap"` -> `2 passed, 140 deselected`.
...
- G2 full NLE disk-format cutover remains blocked until legacy disk-shape replacement policy and final cutover proof are implemented and reproved.
- G0 has owner approval to proceed, but remains blocked on Apple Distribution and 3rd Party Mac Developer Installer identities, signed App Store `.pkg`, strict content-bound `codesign`, content-bound `pkgutil --check-signature`, sandbox smoke, content-bound App Store Connect validation, upload/submission proof, and owner-approved metadata values JSON.
- G1 collect-cache/default promotion remains owner-review gated; any future default change must be one cache at a time with a rollback commit boundary and focused same-fixture proof.
...
- Stop after this completed action item unless the owner explicitly continues.
- If continuing G2, prove legacy disk-shape replacement safety without moving default authority or final cutover wording.
- If continuing G0, collect owner-approved metadata values JSON or configure Apple Distribution / installer identities before package/upload execution.

## Previous Handoff - 2026-06-29 v04.01.28 / G2 NLE Snapshot Standalone Canonical Load Opt-In Proof

### Scope

- Completed the G2 standalone `nle_snapshot` canonical load-source opt-in proof slice.
- Bumped source-app version and project schema from `04.01.27` to `04.01.28`.
...
- Checkpoint code version: `APP_VERSION=04.01.28`.
- Checkpoint project schema version: `PROJECT_SCHEMA_VERSION=04.01.28`.
- NLE audit: `output/manual_verification/latest/nle_snapshot_canonical_load_source_v040128_20260629_1325/nle_persistence_cutover_audit.md`.
- Checkpoint G2 state: `status=blocked`, `prep_ready=true`, `persistence_cutover_ready=false`, overall stoplight `red`, ready/blocked gates `9/3`.
- `nle_snapshot_canonical_load_source_allowed` is now ready for explicit owner-approved standalone snapshot opt-in payloads only.
- Loaded/runtime/reloaded/storage `nle_snapshot` first caption text remains `snapshot canonical first`; legacy `editor_state` first caption text after resave remains `first`.
- Compatibility-only, forged, empty, and ambiguous dual-owner payloads fall back to legacy rows, and top-level/runtime/readback/quarantine payloads do not persist.
- Remaining blocked gates: `runtime_project_state_persistence_allowed`, `legacy_disk_shape_replacement_allowed`, and `final_cutover_ready`.
- This is an explicit opt-in load-source proof only. It is not persisted `_nle_project_state`, legacy disk-shape replacement, final NLE cutover, UI/UX change, or App Store submission proof.
...
- Focused NLE guard: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_project_nle_persistence_guard.py tests/test_nle_persistence_cutover_audit.py tests/test_macos_bundle_runtime_paths.py` -> `27 passed`.
- Audit generation: `QT_QPA_PLATFORM=offscreen ./venv/bin/python tools/audit_nle_persistence_cutover.py --output-dir output/manual_verification/latest/nle_snapshot_canonical_load_source_v040128_20260629_1325` -> `status=blocked`, ready/blocked gates `9/3`, blockers `3`.
- Project/status guard: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_project_context.py tests/test_cp03_cp04_status_ui.py -k "schema or version or project_file_roundtrip or status"` -> `66 passed, 80 deselected`.
- Direct version assertion -> `APP_VERSION=04.01.28`, `PROJECT_SCHEMA_VERSION=04.01.28`.
- `git diff --check -- .` -> pass.
- Three sub-agent reviews were collected for architecture boundary, QE false-positive guards, and editor-workflow wording. Jammini `--status` resolved the active route and Jammini received a bounded scout task, but no fresh physical Jammini result file arrived before this slice closed; `.agents/sentinel/handoffs/20260629-070211-watchdog-handoff-probe.md` remains the latest physical route proof.

### Remaining Risks

- G2 full NLE disk-format cutover remains blocked until runtime-state persistence policy, legacy disk-shape replacement policy, and final cutover proof are implemented and reproved.
- G0 has owner approval to proceed, but remains blocked on Apple Distribution and 3rd Party Mac Developer Installer identities, signed App Store `.pkg`, strict content-bound `codesign`, content-bound `pkgutil --check-signature`, sandbox smoke, content-bound App Store Connect validation, upload/submission proof, and owner-approved metadata values JSON.
- G1 collect-cache/default promotion remains owner-review gated; any future default change must be one cache at a time with a rollback commit boundary and focused same-fixture proof.
...
## Previous Handoff - 2026-06-29 v04.01.27 / G2 Top-Level NLE Canonical Load Opt-In Proof
...
- Completed the G2 canonical load-owner rollback-boundary audit slice.
- Bumped source-app version and project schema from `04.01.23` to `04.01.24`.
- Extended `tools/audit_nle_persistence_cutover.py` and `tests/test_nle_persistence_cutover_audit.py`.
- Generated audit evidence proving a candidate top-level `nle`/`nle_snapshot`/runtime-state canonical-owner payload is stripped back to legacy `editor_state` loading before default load or resave can adopt it.

### Result

- Current code version: `APP_VERSION=04.01.24`.
- Current project schema version: `PROJECT_SCHEMA_VERSION=04.01.24`.
...
- Audit state: `status=blocked`, `app_version=04.01.24`, `prep_ready=true`, `persistence_cutover_ready=false`, and `top_level_nle_compatibility_projection_passed=true`.
...
- Blocked gates remain `canonical_load_owner_change_allowed`, `nle_snapshot_canonical_load_source_allowed`, `runtime_project_state_persistence_allowed`, `legacy_disk_shape_replacement_allowed`, and `final_cutover_ready`.
- Rollback proof: candidate shadow text `rollback candidate shadow text` never reaches default load or resave; both keep first caption text `first`. Resave regenerates approved `nle`/`nle_snapshot` shadow metadata with `canonical_load_owner=legacy_editor_state` and without persisted runtime state or quarantine.
- Owner approval exists for G0 App Store packaging/signing/upload/metadata execution and persisted NLE/UI structure scope, but this slice is audit evidence only. It is not a project load-owner switch, `nle_snapshot` canonical source switch, persisted `_nle_project_state`, legacy `editor_state` replacement, visible UI/UX change, STT/cache default change, App Store package creation, signing, validation, upload, submission, or metadata completion proof.
...
- Audit generation: `./venv/bin/python tools/audit_nle_persistence_cutover.py --output-dir output/manual_verification/latest/nle_load_owner_rollback_boundary_v040124_20260629_1138` -> `status=blocked`, `overall_stoplight=red`, ready/blocked gates `7/5`.
...
- G2 full NLE disk-format cutover remains blocked until canonical load-owner change policy, `nle_snapshot` canonical-source policy, runtime-state persistence policy, legacy disk-shape replacement policy, and final cutover proof are implemented and reproved.
- G0 has owner approval to proceed, but remains blocked on Apple Distribution and 3rd Party Mac Developer Installer identities, signed App Store `.pkg`, strict `codesign`, `pkgutil --check-signature`, sandbox smoke, App Store Connect validation, upload/submission proof, and owner-approved metadata values.
- G1 collect-cache/default promotion remains owner-review gated; any future default change must be one cache at a time with a rollback commit boundary and focused same-fixture proof.

### Next Recommended Action

- Stop after this completed action item unless the owner explicitly continues.
- If continuing G0, use the owner approval to run the next App Store packaging/signing/metadata step only after required identities and metadata values are available, then preserve exact logs.
- If continuing G2, choose a separate owner-approved canonical load-owner or UI-structure slice and prove save/reopen/render/export parity before widening behavior.

## Previous Handoff - 2026-06-29 v04.01.23 / G2 Canonical Load-Owner Gate Matrix Audit
...
- Completed the G0 App Store readiness blocker matrix audit slice.
- Bumped source-app version and project schema from `04.01.21` to `04.01.22`.
- Extended `tools/audit_app_store_readiness.py`, `tools/generate_app_store_metadata_package.py`, and focused App Store readiness tests.
- Generated audit and metadata package evidence that keeps App Store submission blocked unless exact signed-artifact, validation, and owner-metadata proof exists.

### Result

- Current code version: `APP_VERSION=04.01.22`.
- Current project schema version: `PROJECT_SCHEMA_VERSION=04.01.22`.
...
- Audit state: `status=blocked`, `local_packaging_ready=true`, `app_store_submission_ready=false`, overall stoplight `red`, blocker count `17`.
- Blocker groups: `version_lock=0`, `packaging_template=0`, `signed_artifacts=3`, `sandbox_smoke=1`, `app_store_connect=1`, `signing_identities=4`, `owner_metadata=8`, `unknown=0`.
...
- This is audit/package metadata evidence only. It is not package creation, Apple Distribution signing, App Store Connect validation, upload, submission, owner metadata completion, UI/UX change, subtitle-generation change, STT/cache default change, or NLE persistence/load behavior change.

### Evidence

- Compile check: `./venv/bin/python -m py_compile tools/audit_app_store_readiness.py tools/generate_app_store_metadata_package.py tests/test_app_store_readiness_audit.py tests/test_app_store_metadata_package.py core/runtime/config.py core/project/project_format.py tests/test_macos_bundle_runtime_paths.py` -> pass.
- App Store audit/metadata/bundle guards: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_app_store_readiness_audit.py tests/test_app_store_metadata_package.py tests/test_macos_bundle_runtime_paths.py` -> `17 passed`.
- Audit generation: `./venv/bin/python tools/audit_app_store_readiness.py --output-dir output/manual_verification/latest/app_store_readiness_blocker_matrix_v040122_20260629_1100` -> `status=blocked`, overall stoplight `red`, blocker count `17`.
- Metadata package generation: `./venv/bin/python tools/generate_app_store_metadata_package.py --output-dir output/manual_verification/latest/app_store_metadata_owner_input_package_v040122_20260629_1100` -> `not_submission_proof=true`, pending owner-input metadata `8/8`.
- Project/status guard: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_project_context.py tests/test_cp03_cp04_status_ui.py -k "schema or version or project_file_roundtrip or status"` -> `66 passed, 80 deselected`.
...
- G0 remains blocked on Apple Distribution and 3rd Party Mac Developer Installer identities, signed App Store `.pkg`, strict `codesign`, `pkgutil --check-signature`, sandbox smoke, App Store Connect validation, upload/submission proof, and owner-approved metadata values.
- G2 full NLE disk-format cutover remains blocked until exact canonical load-owner scope, rollback/quarantine behavior, and legacy/direct-SRT/roughcut/render-export proof are complete.
- G1 collect-cache/default promotion remains owner-review gated; any future default change must be one cache at a time with a rollback commit boundary and focused same-fixture proof.
...
## Previous Handoff - 2026-06-29 v04.01.21 / G2 Top-Level NLE Gap Projection Coverage Audit

### Scope

- Completed the G2 top-level NLE gap projection coverage audit slice.
- Bumped source-app version and project schema from `04.01.20` to `04.01.21`.
- Extended `core/project/nle_snapshot.py`, `tools/audit_nle_persistence_cutover.py`, and focused NLE persistence tests.
- Generated audit evidence that explicitly projects top-level `nle` captions plus legacy gap rows as non-caption gap metadata while proving default project load still uses legacy rows.

### Result

- Current code version: `APP_VERSION=04.01.21`.
- Current project schema version: `PROJECT_SCHEMA_VERSION=04.01.21`.
...
- Audit generation: `./venv/bin/python tools/audit_nle_persistence_cutover.py --output-dir output/manual_verification/latest/nle_top_level_gap_projection_v040121_20260629_1041` -> `status=blocked`, `gap_projection_coverage_ready_blocked`, `gap_coverage_ready=true`.
...
- Focused NLE audit: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_nle_persistence_cutover_audit.py` -> `6 passed`.
- Focused snapshot/persistence subset: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_project_nle_persistence_guard.py tests/test_project_nle_snapshot.py -k "top_level or readback or direct_srt or snapshot_adapter"` -> `7 passed, 17 deselected`.
- NLE audit/canonical packet/persistence/snapshot/macOS bundle guards: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_nle_persistence_cutover_audit.py tests/test_nle_canonical_load_owner_review_packet.py tests/test_project_nle_persistence_guard.py tests/test_project_nle_snapshot.py tests/test_macos_bundle_runtime_paths.py` -> `39 passed, 4 subtests passed`.
- Project/status guard: `QT_QPA_PLATFORM=offscreen ./venv/bin/python -m pytest -q tests/test_project_context.py tests/test_cp03_cp04_status_ui.py -k "schema or version or project_file_roundtrip or status"` -> `66 passed, 80 deselected`.
...
## Previous Handoff - 2026-06-29 v04.01.20 / G2 Top-Level NLE Compatibility Projection Audit
...
- Completed the G0 source quick QA baseline before packaging slice.
- Bumped source-app version and project schema from `04.01.16` to `04.01.17`.
- Refreshed source-app quick QA baseline and updated current docs/archive pointers.

### Result

- Current code version: `APP_VERSION=04.01.17`.
- Current project schema version: `PROJECT_SCHEMA_VERSION=04.01.17`.
- Quick QA baseline: `output/manual_verification/latest/qa_suite_quick_v040117_20260629_0929/suite_result.md`.
- Quick QA result: `profile=quick`, `scenario_count=1`, scenario `editor_compact_macau`, `passed=1`, `failed=0`, started `2026-06-29 09:30:03`, finished `2026-06-29 09:30:13`.
- This is source-app editor workflow baseline only. It is not signed package, sandbox smoke, App Store validation/upload/submission, owner metadata, full QA, real-media STT quality, roughcut, or X5 rolling proof.
...
- Quick QA: `AI_SUBTITLE_STUDIO_QA_USE_SOURCE=1 QT_QPA_PLATFORM=offscreen ./venv/bin/python tools/qa_suite_runner.py quick --output-dir output/manual_verification/latest/qa_suite_quick_v040117_20260629_0929` -> `failed_count=0`.
...
## Previous Handoff - 2026-06-29 v04.01.16 / G0 App Store Metadata Owner-Input Package
...
- Continued the owner goal to move AI Subtitle Studio toward a video-editor/NLE structure by applying Taption-style final/preview subtitle surface isolation.
- `core.engine.subtitle_live_editor_feed.SubtitleLiveEditorFeed` now exposes explicit `final_surface_segments` and `preview_lane_segments`, and marks `combined_segments` as diagnostic/candidate-lane only.
- `ui.editor.editor_segments_timeline_context.EditorSegmentsTimelineContextMixin._video_subtitle_live_preview_context(...)` now keeps STT/subtitle drafts out of the video subtitle overlay; timeline/editor candidate lanes still keep live previews.
- Added `tools/audit_nle_final_preview_isolation.py`.
- No UI layout/labels/colors/menus/popups, subtitle generation policy, STT/STT2/default-cache policy, persisted `.aissproj` NLE fields, App Store packaging/signing/upload, DMG, runtime undo/redo UI, or per-pixel NLE write behavior changed.

### Results

- Audit: `output/manual_verification/latest/nle_final_preview_isolation_20260628/nle_final_preview_isolation.md`
- `ready=true`; final overlay/global canvas/save-style surfaces are `confirmed_final_rows_only`; combined live feed is `diagnostic_candidate_lane_only`; video overlay ignores live preview context.
- NAS HeyDealer preflight: `output/manual_verification/latest/nle_final_preview_isolation_nas_preflight_20260628/reference_fixture_availability.md`; ready `true`, media/SRT exist `true/true`, clipped reference rows `89`.
- NAS HeyDealer current-head regression: `output/manual_verification/latest/nle_final_preview_isolation_nas_heydealer_20260628/acceptance/reference_benchmark_acceptance.md`
- Run `.codex_work/benchmarks/subtitle_pipeline_variants/20260628_195502/benchmark_results.json`: accepted `true`, elapsed `46.228s`, raw/final/reference `58/56/89`, quality/text/timing `93.766/94.267/0.5808s`, final invalid/non-monotonic/overlap `0/0/0`, final last end/duration bound `180.0/180.0`, short/long `0/0`, global max-active `1`, STT1/STT2/word selected `21/37/7`.
- Timeout audit: `output/manual_verification/latest/stt_worker_timeout_compare_nle_final_preview_isolation_nas_20260628/stt_worker_timeout_audit.md`; timeout detected `false`.

### Jammini

- Route probe: `.agents/sentinel/handoffs/20260628-194923-watchdog-handoff-probe.md`
- Scout: `.agents/sentinel/handoffs/20260628-104950-next-nle-taption-runtime-contract-scout.md`
- Dex classification: rejected/deferred the scout's playhead-jump proposal because playhead isolation was already completed in `nle_playhead_jump_isolation`. Adopted the directly inspected final/preview isolation gap instead.
...
- `QT_QPA_PLATFORM=offscreen ./venv/bin/python tools/verify_reference_fixture_availability.py --output-dir output/manual_verification/latest/nle_final_preview_isolation_nas_preflight_20260628` -> ready `true`.
- `QT_QPA_PLATFORM=offscreen ./venv/bin/python tools/benchmark_subtitle_pipeline_variants.py --suite modes --variants mode_high --media ... --reference-srt ... --start-sec 0 --duration-sec 180 --keep-artifacts` -> `.codex_work/benchmarks/subtitle_pipeline_variants/20260628_195502/benchmark_results.json`.
- `./venv/bin/python tools/evaluate_reference_benchmark_acceptance.py .codex_work/benchmarks/subtitle_pipeline_variants/20260628_195502/benchmark_results.json --output-dir output/manual_verification/latest/nle_final_preview_isolation_nas_heydealer_20260628/acceptance` -> accepted `true`.
- `./venv/bin/python tools/audit_stt_worker_timeout.py .codex_work/benchmarks/subtitle_pipeline_variants/20260628_194124/benchmark_results.json .codex_work/benchmarks/subtitle_pipeline_variants/20260628_195502/benchmark_results.json --output-dir output/manual_verification/latest/stt_worker_timeout_compare_nle_final_preview_isolation_nas_20260628` -> timeout detected `false`.

### Known Notes

- This slice intentionally changes the video subtitle overlay rule: live STT/subtitle drafts remain available in timeline/editor candidate lanes, but not in the video subtitle overlay.
- Persisted NLE disk fields, UI redesign, automatic relink UI/dialog behavior, per-pixel writes, QML/GPU default timeline surfaces, detector-threshold changes, App Store packaging/submission work, and STT/default-cache policy changes remain blocked until explicit owner approval and compatibility proof exist.
...
## Previous Handoff - 2026-06-28 NLE Relink Parity Verification

### Scope

- Continued the owner goal to move AI Subtitle Studio toward a video-editor/NLE structure by proving project-load/media relink parity between editor media files and runtime `NLEProjectState` snapshot assets/clips.
- Added `core.project.nle_project_state.nle_media_relink_parity_signature(...)` and `assert_nle_media_relink_parity(...)`.
- Added `tools/audit_nle_relink_parity.py` and `tests/test_nle_relink_verification.py`.
- The audit proves updated relink media paths match across project media, NLE clips, and NLE assets; rejects editor/timeline media path drift; rejects runtime fps drift; keeps operation journal count `0`; and keeps persisted storage clean of `_nle_project_state`, `nle`, and `nle_snapshot`.
- No UI layout/labels/colors/menus/popups, subtitle generation policy, STT/STT2/default-cache policy, persisted `.aissproj` NLE fields, project-storage relink schema, App Store packaging/signing/upload, DMG, runtime undo/redo UI, or per-pixel NLE write behavior changed.

### Results

- Audit: `output/manual_verification/latest/nle_relink_parity_20260628/nle_relink_parity.md`
- `ready=true`; project/NLE media counts `1/1/1`; project/runtime/sequence fps `30.0/30.0/30.0`; project/sequence duration `6.0/6.0`; path drift rejected `true`; operation journal count `0`; storage forbidden key count `0`.
- NAS HeyDealer preflight: `output/manual_verification/latest/nle_relink_parity_nas_preflight_20260628/reference_fixture_availability.md`; ready `true`, media/SRT exist `true/true`, clipped reference rows `89`.
- NAS HeyDealer current-head regression: `output/manual_verification/latest/nle_relink_parity_nas_heydealer_20260628/acceptance/reference_benchmark_acceptance.md`
- Run `.codex_work/benchmarks/subtitle_pipeline_variants/20260628_194124/benchmark_results.json`: accepted `true`, elapsed `48.693s`, raw/final/reference `58/56/89`, quality/text/timing `93.766/94.267/0.5808s`, final invalid/non-monotonic/overlap `0/0/0`, final last end/duration bound `180.0/180.0`, short/long `0/0`, global max-active `1`, STT1/STT2/word selected `21/37/7`.
- Timeout audit: `output/manual_verification/latest/stt_worker_timeout_compare_nle_relink_parity_nas_20260628/stt_worker_timeout_audit.md`; timeout detected `false`.

### Jammini

- Scout: `.agents/sentinel/handoffs/20260628-103348-next-nle-taption-runtime-contract-scout.md`
- Dex classification: accepted the relink parity validation direction but narrowed it to helper/audit/test evidence. Runtime fallback recovery, automatic relink UI/dialog behavior, persisted NLE fields, project-storage relink schemas, per-pixel writes, STT/default promotion, and App Store packaging remain deferred.
...
- `QT_QPA_PLATFORM=offscreen ./venv/bin/python tools/verify_reference_fixture_availability.py --output-dir output/manual_verification/latest/nle_relink_parity_nas_preflight_20260628` -> ready `true`.
- `QT_QPA_PLATFORM=offscreen ./venv/bin/python tools/benchmark_subtitle_pipeline_variants.py --suite modes --variants mode_high --media ... --reference-srt ... --start-sec 0 --duration-sec 180 --keep-artifacts` -> `.codex_work/benchmarks/subtitle_pipeline_variants/20260628_194124/benchmark_results.json`.
- `./venv/bin/python tools/evaluate_reference_benchmark_acceptance.py .codex_work/benchmarks/subtitle_pipeline_variants/20260628_194124/benchmark_results.json --output-dir output/manual_verification/latest/nle_relink_parity_nas_heydealer_20260628/acceptance` -> accepted `true`.
- `./venv/bin/python tools/audit_stt_worker_timeout.py .codex_work/benchmarks/subtitle_pipeline_variants/20260628_192507/benchmark_results.json .codex_work/benchmarks/subtitle_pipeline_variants/20260628_194124/benchmark_results.json --output-dir output/manual_verification/latest/stt_worker_timeout_compare_nle_relink_parity_nas_20260628` -> timeout detected `false`.

### Known Notes

- This slice adds a reusable validation contract; it does not perform automatic media replacement, alter relink dialogs, or approve persisted project relink schemas.
...
## Previous Handoff - 2026-06-28 NLE Selection Sync Validation
