...
[![App Version](https://img.shields.io/badge/app-04.01.27-0A84FF?style=for-the-badge)](#)
[![Release](https://img.shields.io/badge/release-v04.01.27-30D158?style=for-the-badge)](../release_notes/RELEASE_v04.01.27.md)
...
- The current G2 NLE checkpoint adds an explicit top-level `nle` canonical load opt-in proof: paired `nle` / `nle_snapshot` rows can drive project load only under owner-approved policy, while legacy `editor_state` remains preserved for rollback. No `nle_snapshot` standalone canonical sourcing, persisted runtime state, visible UI/UX change, or final disk-format cutover is claimed by this checkpoint.
...
| Handoff document version | `04.01.27-source-app` |
...
The current release checkpoint is [`RELEASE_v04.01.27.md`](../release_notes/RELEASE_v04.01.27.md). The repository keeps only the most recent release notes needed for handoff continuity, and the five handoff documents should summarize only the current state plus the immediately previous release relationship.
