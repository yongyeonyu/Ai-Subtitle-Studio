...
[![App Version](https://img.shields.io/badge/app-04.01.28-0A84FF?style=for-the-badge)](#)
[![Release](https://img.shields.io/badge/release-v04.01.28-30D158?style=for-the-badge)](../release_notes/RELEASE_v04.01.28.md)
...
- The current G2 NLE checkpoint adds an explicit standalone `nle_snapshot` canonical load-source opt-in proof: approved `nle_snapshot` rows can drive project load only under owner-approved policy, while legacy `editor_state` remains preserved for rollback. No persisted runtime state, visible UI/UX change, legacy disk-shape replacement, or final disk-format cutover is claimed by this checkpoint.
...
| Handoff document version | `04.01.28-source-app` |
...
The current release checkpoint is [`RELEASE_v04.01.28.md`](../release_notes/RELEASE_v04.01.28.md). The repository keeps only the most recent release notes needed for handoff continuity, and the five handoff documents should summarize only the current state plus the immediately previous release relationship.
