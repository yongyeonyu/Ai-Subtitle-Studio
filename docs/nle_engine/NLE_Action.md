...
<!--
Document-Version: 04.01.29-nle-action
...
- Open status: persisted NLE/UI structure scope was owner-approved on 2026-06-28 and reconfirmed on 2026-06-29. Approved `nle_snapshot` compatibility metadata, top-level `nle` shadow metadata persistence, explicit top-level `nle` canonical load opt-in, explicit standalone `nle_snapshot` canonical load-source opt-in, and explicit supplemental `_nle_project_state` persistence opt-in are now available only for explicitly marked projects using owner-approved `nle_persistence` policy. Legacy disk-shape replacement, final cutover, and per-pixel drag writes remain explicitly out of scope until separate compatibility proof exists.
- Latest G2 runtime `_nle_project_state` persistence opt-in audit: `v04.01.29` writes audit evidence at `output/manual_verification/latest/nle_runtime_state_persistence_v040129_20260629_140053/nle_persistence_cutover_audit.md`. The audit state is `blocked`, `prep_ready=true`, `persistence_cutover_ready=false`, `overall_stoplight=red`, ready/blocked gates are `10/2`, current canonical load owner is `nle_snapshot` for explicit opt-in payloads only, `nle_snapshot_canonical_load_source_allowed=ready`, and `runtime_project_state_persistence_allowed=ready`. Loaded/runtime/reloaded/storage snapshot/runtime first caption text stays `runtime persisted snapshot first`; legacy `editor_state` first caption text after resave remains `first`; cache-hit read/resave hydrates runtime state; default project load/save/export authority remains unchanged; top-level/readback/quarantine payloads do not persist. This is not a default authority switch, Direct SRT precedence change, roughcut sidecar change, disk-format cutover, legacy `editor_state` replacement, or visible UI/UX change.
...
- `NLEProjectState` starts as an in-memory editor/session owner; persist `_nle_project_state` only as an explicit owner-approved supplemental runtime-state payload tied to standalone `nle_snapshot` canonical load-source opt-in.
