...
- Open status: persisted NLE/UI structure scope was owner-approved on 2026-06-28. Approved `nle_snapshot` compatibility metadata persistence is now available only for explicitly marked projects using `nle_persistence.persist_snapshot=true` and `approval=owner_approved_20260628`; top-level `nle`, persisted `_nle_project_state`, canonical load ownership from `nle_snapshot`, and per-pixel drag writes remain explicitly out of scope until separate compatibility proof exists.
- Latest direct SRT and roughcut read-back parity proof: `output/manual_verification/latest/nle_direct_srt_roughcut_readback_parity_20260629_0003/nle_persistence_cutover_audit.md`; `prep_ready=true`, approved snapshot persistence `ready=true`, snapshot persisted `true`, legacy rows stable `true`, approved read-back parity stable `true`, direct SRT open records a runtime `surface=direct_srt_open` drift report without overwriting SRT timing/text, intentionally corrupted roughcut-sidecar persisted markers are detected while render/export parity and roughcut sidecar parity remain stable, runtime reports stay non-persistent, top-level `nle`/runtime/quarantine storage remains blocked, operation roundtrip all passed across `11` families, and full cutover remains `persistence_cutover_ready=false` with blockers `top_level_nle_payload_not_cut_over`, `runtime_nle_project_state_must_remain_runtime_only`, and `legacy_disk_shape_required_for_full_cutover`.
...
- `NLEProjectState` starts as an in-memory editor/session owner; do not persist top-level `nle` or `_nle_project_state` fields into `.aissproj`.
- Persist `nle_snapshot` only as owner-approved compatibility metadata when the project explicitly carries the `owner_approved_20260628` persistence policy; legacy editor rows remain canonical for load/save.
