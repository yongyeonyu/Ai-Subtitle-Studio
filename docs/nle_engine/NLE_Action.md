...
<!--
Document-Version: 04.01.07-nle-action
...
- Latest G3 live runtime observability strong-evidence gate slice: `tools/remote_verify.py live-nle-proof` starts a guided subtitle run, polls compact `guided-subtitle-status` samples, can capture existing-window snapshots, and writes `live_nle_runtime_proof.md/json`, `status_samples.json`, and `observability_samples.jsonl`. The report schema is `ai_subtitle_studio.live_nle_runtime_proof.v2`, and the default pass gate requires `VAD`/`STT1`/`STT2` to appear in at least two distinct active pre-final polls plus generation completion. It blocks missing/insufficient pre-final observations, non-compact payloads, raw runtime payload leakage, non-final authority drift, and live projection budget drift on active samples. This is proof tooling only; it does not claim real-media visual proof or final quality/speed acceptance until a fixture run is recorded.
