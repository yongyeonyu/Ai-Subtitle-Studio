...
<!--
Document-Version: 04.01.06-nle-action
...
- Latest G3 live runtime observability proof harness slice: `tools/remote_verify.py live-nle-proof` starts a guided subtitle run, polls compact `guided-subtitle-status` samples, can capture existing-window snapshots, and writes `live_nle_runtime_proof.md/json` plus `status_samples.json`. It blocks missing pre-final `VAD`/`STT1`/`STT2` counts, raw runtime payload leakage, non-final authority drift, and live projection budget drift on active samples. This is proof tooling only; it does not claim real-media visual proof or final quality/speed acceptance until a fixture run is recorded.
