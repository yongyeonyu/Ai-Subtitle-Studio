...
<!--
Document-Version: 04.01.28-nle-action
...
- Latest G2 standalone NLE snapshot canonical load-source audit: `v04.01.28` writes audit evidence at `output/manual_verification/latest/nle_snapshot_canonical_load_source_v040128_20260629_1325/nle_persistence_cutover_audit.md`. The audit state is `blocked`, `prep_ready=true`, `persistence_cutover_ready=false`, `overall_stoplight=red`, ready/blocked gates are `9/3`, current canonical load owner is `nle_snapshot` for explicit opt-in payloads only, and `nle_snapshot_canonical_load_source_allowed=ready`. Loaded/runtime/reloaded/storage `nle_snapshot` first caption text stays `snapshot canonical first`; legacy `editor_state` first caption text after resave remains `first`; compatibility-only/forged/empty/ambiguous dual-owner payloads fail closed to legacy; top-level/runtime/readback/quarantine payloads do not persist. This is not a default load-source switch, disk-format cutover, runtime-state persistence, legacy `editor_state` replacement, or visible UI/UX change.
