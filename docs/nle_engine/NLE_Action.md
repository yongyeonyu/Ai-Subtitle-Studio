...
<!--
Document-Version: 04.01.20-nle-action
...
- Open status: persisted NLE/UI structure scope was owner-approved on 2026-06-28 and reconfirmed on 2026-06-29. Approved `nle_snapshot` compatibility metadata and top-level `nle` shadow metadata persistence are now available only for explicitly marked projects using `nle_persistence.persist_snapshot=true`, `nle_persistence.persist_top_level_nle=true`, and `approval=owner_approved_20260628`; persisted `_nle_project_state`, canonical load ownership from `nle`/`nle_snapshot`, and per-pixel drag writes remain explicitly out of scope until separate compatibility proof exists.
- Latest G2 top-level NLE compatibility projection audit: `v04.01.20` writes audit evidence at `output/manual_verification/latest/nle_top_level_compatibility_projection_v040120_20260629_1018/nle_persistence_cutover_audit.md`. The audit state is `compatibility_projection_partial_blocked`, `not_runtime_change=true`, default project load still uses `legacy_editor_state`, explicit projection source is `top_level_nle_shadow_metadata`, explicit projection differs from default legacy captions, explicit projection caption/gap count is `2/0`, default row/caption/gap count is `3/2/1`, `gap_coverage_ready=false`, `canonical_load_owner_change_allowed=false`, and `disk_format_cutover_allowed=false`. This is not a canonical load-owner switch or disk-format cutover; it identifies `top_level_nle_projection_gap_coverage_missing` as the next blocker before any load-owner change.
...
- `NLEProjectState` starts as an in-memory editor/session owner; do not persist `_nle_project_state` fields into `.aissproj`.
- Persist `nle_snapshot` and top-level `nle` shadow metadata only as owner-approved compatibility metadata when the project explicitly carries the `owner_approved_20260628` persistence policy; legacy editor rows remain canonical for load/save.
- On load, compare any persisted approved `nle_snapshot` against a freshly rebuilt snapshot through `_nle_snapshot_readback_parity`; direct SRT open may compare the persisted snapshot against the exact imported SRT editor rows with `surface=direct_srt_open`. Treat drift as runtime diagnostic evidence only, rebuild from legacy/direct SRT rows, and never make the persisted snapshot the canonical load owner.
