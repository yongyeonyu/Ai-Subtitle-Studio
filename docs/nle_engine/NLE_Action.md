...
<!--
Document-Version: 04.01.27-nle-action
...
- Open status: persisted NLE/UI structure scope was owner-approved on 2026-06-28 and reconfirmed on 2026-06-29. Approved `nle_snapshot` compatibility metadata, top-level `nle` shadow metadata persistence, and explicit top-level `nle` canonical load opt-in are now available only for explicitly marked projects using owner-approved `nle_persistence` policy. Persisted `_nle_project_state`, `nle_snapshot` standalone canonical sourcing, legacy disk-shape replacement, final cutover, and per-pixel drag writes remain explicitly out of scope until separate compatibility proof exists.
- Latest G2 top-level NLE canonical load opt-in audit: `v04.01.27` writes audit evidence at `output/manual_verification/latest/nle_canonical_load_opt_in_v040127_20260629_1248/nle_persistence_cutover_audit.md`. The audit state is `blocked`, `prep_ready=true`, `persistence_cutover_ready=false`, `overall_stoplight=red`, ready/blocked gates are `8/4`, current canonical load owner is `top_level_nle_shadow_metadata` for explicit opt-in payloads only, and `canonical_load_owner_change_allowed=ready`. Loaded/runtime/reloaded/storage `nle`/storage `nle_snapshot` first caption text all stay `nle canonical first`; legacy `editor_state` first caption text after resave remains `first`; companion drift fails closed; runtime/readback/quarantine payloads do not persist. This is not `nle_snapshot` standalone canonical sourcing, disk-format cutover, runtime-state persistence, legacy `editor_state` replacement, or visible UI/UX change.
...
- Persist `nle_snapshot` and top-level `nle` metadata only when the project explicitly carries owner-approved `nle_persistence` policy; legacy editor rows remain the rollback source for load/save.
- Top-level `nle` may be the load source only under the explicit canonical-load opt-in policy and only when paired `nle_snapshot` rows agree. `nle_snapshot` alone is not a canonical load source.
- On load, compare any persisted approved `nle_snapshot` against the paired top-level `nle` or a freshly rebuilt snapshot as the active gate requires; direct SRT open may compare the persisted snapshot against the exact imported SRT editor rows with `surface=direct_srt_open`. Treat drift as fail-closed diagnostic evidence, rebuild from legacy/direct SRT rows, and never make the persisted snapshot the standalone canonical load owner.
