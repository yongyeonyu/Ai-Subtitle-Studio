...
| Render plans | `core/roughcut/renderer_skeleton.py`, `core/roughcut/render_executor.py`, `ui/roughcut/roughcut_export.py` | `RenderCommandPlan`, `_render_plan.json`, render result manifest | Model as `RenderPlan`; output duration and sidecar metadata must remain byte/metadata-equivalent before routing changes. `build_concat_render_plan_from_snapshot` is the current parity adapter; UI export routing still stays with `ui/roughcut/roughcut_export.py`. |
