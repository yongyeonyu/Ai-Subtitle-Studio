...
`tools/remote_verify.py live-nle-proof` owns the current G3 proof harness for runtime observability. It consumes compact `guided-subtitle-status` snapshots, optional existing-window screenshots, and the live projection budget report to prove whether `VAD`, `STT1`, and `STT2` were observed in at least two distinct active pre-final polls before final completion. It writes redacted summary JSON, JSON sample arrays, and JSONL observation samples, but must not become a raw runtime-row transport, final subtitle owner, project persistence path, or replacement for same-media quality/speed validation.
