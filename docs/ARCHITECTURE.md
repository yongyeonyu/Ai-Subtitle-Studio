...
- `tools/generate_nle_canonical_load_owner_review_packet.py`: G2 NLE canonical load-owner review packet generator for historical owner-review packets.
- `tools/audit_nle_persistence_cutover.py`: G2 NLE persistence audit; now proves explicit top-level `nle` compatibility projection, snapshot canonical load-source, supplemental runtime-state persistence, legacy-compatible `editor_state` projection, and final source-app persistence load-owner opt-in while keeping UI/App Store/STT proof separate.
...
`tools/remote_verify.py live-nle-proof` owns the current G3 proof harness for runtime observability. It consumes compact `guided-subtitle-status` snapshots, optional existing-window screenshots, and the live projection budget report to prove whether `VAD`, `STT1`, and `STT2` were observed in at least two distinct active pre-final polls before final completion. The representative NAS-derived HeyDealer first-180s proof passed at `output/manual_verification/latest/g3_live_nle_real_media_observability_timeout20_20260629/live_nle_runtime_proof.md`, but this is runtime/status/snapshot evidence only. The harness writes redacted summary JSON, JSON sample arrays, and JSONL observation samples, but must not become a raw runtime-row transport, final subtitle owner, project persistence path, or replacement for same-media quality/speed, save/reopen, final export, or global-canvas validation.
...
- `Sequence`: an ordered read-only timeline view with id, name, timebase, duration, tracks, source project references, and non-caption gap metadata. The source-app default sequence mirrors the current project timeline.
...
- `GapSegment`: a non-caption gap row in sequence time with frame range, optional clip reference, and legacy metadata copied read-only from project/editor rows. Gap segments are compatibility metadata and must not become final overlay, SRT, or exported caption rows.
...
- `NLEProjectState`: a runtime-only mutable project state hydrated from the legacy payload. It can project editor rows back to the existing legacy save shape, but it must not persist as `_nle_project_state`. `nle_snapshot` and top-level `nle` shadow metadata may persist only as approved compatibility metadata and are not canonical load state; `_nle_snapshot_readback_parity` is runtime-only and stripped before storage. The top-level `nle` projection audit is explicit compatibility evidence only; default project load still uses legacy `editor_state` rows until separate rollback-boundary and load-owner proof is complete.
