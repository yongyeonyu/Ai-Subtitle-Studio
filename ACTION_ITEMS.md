...
- 2026-05-23: STT native hot-path slice completed. Added Swift/Python audio chunk manifest signature caching and selective STT2 straggler handling so the final stalled Fast-STT2 chunk keeps the STT1 candidate instead of forcing a full fallback replay. X5 High 180s benchmark completed with `quality_score=87.556` and `elapsed_sec=101.223`. Artifact: `output/manual_verification/latest/20260523_action_native_stt_manifest_straggler/verification_summary.md`.
- 2026-05-23: Fast-STT2 aggressive straggler slice completed. Added Fast-STT2-specific WhisperKit concurrency defaults (`8`, max `10`), STT2 ratio straggler cutoff (`>=60%` received), and word-precision ratio cutoff (`>=90%` received). X5 High 180s benchmark completed with `quality_score=87.318` and `elapsed_sec=117.401`. Artifact: `output/manual_verification/latest/20260523_fast_stt2_aggressive_straggler/verification_summary.md`.
