...
- [ ] Reduce repeated JSON serialization for size estimation and cache bookkeeping.
  ...
  Additional progress: `core/personalization/lora_store_bundle.py` now routes bundle size estimation through the shared compact-bytes JSON helper and reuses the same encoded manifest/payload bytes across archive retries, while `core/project/project_context.py` hashes subtitle segment signatures from compact JSON bytes directly instead of building an intermediate text string and encoding it again. Runtime disk-cache pruning now has a Swift-native worker path for cache-pressure cleanup, and both the Swift path and Python fallback use oldest-first heaps instead of full sorted file-entry copies. Repeated Swift core worker requests now reuse the already-running worker before rechecking CLI paths, cache resolved CLI hits and misses briefly for repeated native helper calls, cache Swift-runtime env decisions, cache default runtime disk-cache roots and resolved runtime cache path keys for stable config paths, reuse the psutil process handle for RSS polling, cache runtime-trim/native-prune callables, cache runtime acceleration callouts, keep runtime resource psutil module/process handles after coordinator init, count STT LoRA learning events without list materialization, and avoid copying snapshot resource maps during pressure-stage checks to reduce filesystem/stat/process-object/import/list-allocation overhead. Shared native JSON helpers now also cache the normalized enable/disable env decision and native-dump option bitmasks, avoid empty-output sentinel allocation plus redundant bytearray-to-bytes conversion, and cover autopilot stable hashes/JSONL bytes, streaming gzip/zstd JSONL paths, cache-manager hot lookups, recovery settings digests, STT learning event appends, and STT LoRA bundle JSON/checksum bytes so those paths avoid repeated stdlib JSON text assembly, duplicate dict presorting, unnecessary settings/model hash dict copies, full compressed-cache materialization, and encode/decode round trips when the native JSON extension is available.
