...
2. Keep both `stt_recheck_collect_cache_enabled=false` and `stt_primary_collect_cache_enabled=false` by default. The representative HeyDealer first-180s cache-write plus cache-hit replay is now strict-accepted; the next gate is explicit owner review before any default promotion.
3. Use the archived tail-collapse-fixed strict synthetic write/hit replay as generated-fixture evidence only. The production-facing evidence is the latest strict HeyDealer write/hit pair plus readiness refresh, not the generated fixture by itself.
4. Keep the benchmark-window projection repair narrow: it aligns final hypothesis rows to the same requested first-180s benchmark window as the reference rows and must not be used to loosen runtime subtitle stability or save/export guards.
