...
- Current strict real-media collect-cache backfill acceptance: `output/manual_verification/latest/stt_cache_tail_bound_fix_20260628_1048/`; write/hit runs are accepted, elapsed `46.073s -> 1.266s`, final invalid/non-monotonic/overlap `0/0/0`, global max active `1`, and provider calls `true -> false`.
- Current production default decision: `stt_primary_collect_cache_enabled=false` and `stt_recheck_collect_cache_enabled=false` remain unchanged until explicit owner review approves default promotion.
- Current real-media availability/proof family: latest NAS HeyDealer first-180s preflight and strict benchmark artifacts are recorded in `test_result.md` and `COMPLETED_ACTION_ITEMS.md#stt2--word-precision-generation-latency-profiling-and-accuracy-preserving-trim`.
- Completed NLE regression proof bullets were removed from this active queue. Use `COMPLETED_ACTION_ITEMS.md#source-app-nle-runtime-adoption-and-migration-status`, `test_result.md`, and `output/manual_verification/latest/` for completed slice history.
- Generated/X5 evidence remains supporting evidence only. NAS HeyDealer first 180 seconds remains the owner-level gate for production-facing latency/default decisions.
...
- Native migration is not an active direction for this repository. Keep the current Python/PyQt6 source app as the working product line.
- Completed NLE/runtime/documentation slices are archived in `COMPLETED_ACTION_ITEMS.md`, with detailed proof retained in `NLE_Action.md`, `docs/HANDOFF.md`, `test_result.md`, and `output/manual_verification/latest/`. Do not duplicate completed slice summaries back into this active queue.
- Persisted NLE project fields remain gated. Any actual persisted NLE disk-format change still requires a fresh compatibility gate and explicit owner approval.
- The next gate before adopting any new NLE mutation source remains fresh owner-map plus Taption release-commit/no-per-pixel-write/final-overlap/global-canvas/save-reopen proof.
- Revisit native migration, Swift rewrite, QML/GPU timeline defaults, per-pixel NLE writes, or App Store packaging only if the owner explicitly reopens that scope with a new acceptance gate.
