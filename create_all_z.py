...
"""create_all_z.py
PHASE1-B / v02.02.00
Hotfix: route save signals in editor_lifecycle.py to module-level _save_srt_impl.
"""
...
def _remove_block(text: str, anchor: str) -> str:
...
def patch_editor_lifecycle(text: str) -> str:
    ...
    # insert module-level helper once
    if 'def _save_srt_impl(srt_path, segments):' not in text:
        insert_at = text.find('\nclass ')
        if insert_at == -1:
            insert_at = len(text)
        text = text[:insert_at] + '\n' + HELPER + '\n' + text[insert_at:]
    ...
    # normalize signal bindings
    replacements = [
        (
            'editor.sig_save.connect(lambda segs: self._save_srt(srt_path, segs)); editor.sig_auto_save.connect(lambda segs: self._save_srt(srt_path, segs));',
            'editor.sig_save.connect(lambda segs, p=srt_save_path: _save_srt_impl(p, segs)); editor.sig_auto_save.connect(lambda segs, p=srt_save_path: _save_srt_impl(p, segs));',
        ),
        (
    ...
    # remove stale methods if they exist
    for anchor in [
        '    def _save_srt_dispatch(self, editor, srt_path, segments):',
        '    def _save_srt(self, srt_path, segments):',
    ]:
        if anchor in text:
            text = _remove_block(text, anchor)
            changed = True

    # remove misplaced top-level _save_srt if any
    top_anchor = '\ndef _save_srt(self, srt_path, segments):'
    if top_anchor in text:
        start = text.find(top_anchor) + 1
        next_class = text.find('\nclass ', start + 1)
        next_def = text.find('\ndef ', start + 1)
        candidates = [x for x in (next_class, next_def) if x != -1]
        end = min(candidates) if candidates else len(text)
