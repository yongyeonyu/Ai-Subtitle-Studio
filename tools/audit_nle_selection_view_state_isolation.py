...
SCHEMA = "ai_subtitle_studio.nle_selection_view_state_isolation.v1"
AUDIT_ID = "nle_selection_view_state_isolation_20260628"
FORBIDDEN_CALLS = {
...
REQUIRED_TOKENS = {
    "ui/editor/editor_segments_manual_edits.py": {
        "EditorSegmentsManualEditsMixin._on_selection_changed": (
            "_on_cursor_moved",
            "textCursor().hasSelection",
            "editor_popup.close_popup",
        ),
        "EditorSegmentsManualEditsMixin._on_cursor_moved": (
            "_subtitle_memory_cache",
            "find_segment_for_line_lookup",
            "self.timeline.set_active",
            "self.timeline.set_playhead",
            "_schedule_cursor_video_seek",
            "_highlighter.set_current_line",
        ),
    },
    "ui/timeline/timeline_canvas.py": {
        "TimelineCanvas.set_active": (
            "self.active_seg_start = normalized_sec",
            "_sync_active_segment_key",
            "_update_dirty_rect",
        ),
        "TimelineCanvas.clear_active_visual": (
    ...
    "ui/timeline/timeline_widget.py": {
        "TimelineWidget.set_active": (
            "self.canvas.set_active",
            "ensure_sec_visible",
            "_sync_playhead_overlay",
        ),
    },
}
METHODS = (
    ("ui/editor/editor_segments_manual_edits.py", "EditorSegmentsManualEditsMixin", "_on_selection_changed"),
    ("ui/editor/editor_segments_manual_edits.py", "EditorSegmentsManualEditsMixin", "_on_cursor_moved"),
    ("ui/timeline/timeline_canvas.py", "TimelineCanvas", "set_active"),
    ("ui/timeline/timeline_canvas.py", "TimelineCanvas", "clear_active_visual"),
    ("ui/timeline/timeline_widget.py", "TimelineWidget", "set_active"),
)
...
def build_nle_selection_view_state_isolation_report() -> dict[str, Any]:
    method_rows = [
        _method_contract(ROOT / rel_path, class_name, method_name)
        for rel_path, class_name, method_name in METHODS
    ]
    ...
    return {
        ...
        "selection_view_state_only_contract": True,
        "model_validation_allowed": False,
        "project_save_allowed": False,
        "nle_write_allowed": False,
        "primary_row_rewrite_allowed": False,
        "nas_validation_required": False,
        "method_contracts": method_rows,
        "token_contracts": token_rows,
        "blocked_scope": [
            "selection_hover_highlight_must_not_append_nle_operation_journal",
            "selection_hover_highlight_must_not_save_project_or_write_project_file",
            "selection_hover_highlight_must_not_rewrite_primary_canvas_or_global_rows",
            "selection_hover_highlight_must_not_run_subtitle_validation_or_finalize_edits",
            "selection_hover_highlight_must_not_change_ui_layout_labels_menus",
        ],
    }


def write_nle_selection_view_state_isolation_report(output_dir: Path, report: dict[str, Any]) -> tuple[Path, Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    json_path = output_dir / "nle_selection_view_state_isolation.json"
    markdown_path = output_dir / "nle_selection_view_state_isolation.md"
    json_path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    lines = [
        "# NLE Selection View-State Isolation Audit",
        "",
        f"- Schema: `{report['schema']}`",
        f"- Ready: `{report['ready']}`",
        f"- Runtime change applied: `{report['runtime_change_applied']}`",
        f"- Selection view-state-only contract: `{report['selection_view_state_only_contract']}`",
        ...
        f"- Primary row rewrite allowed: `{report['primary_row_rewrite_allowed']}`",
...
def main(argv: list[str] | None = None) -> int:
    ...
    write_nle_selection_view_state_isolation_report(args.output_dir, report)
