...
SCHEMA = "ai_subtitle_studio.nle_time_window_view_decoupling.v1"
AUDIT_ID = "nle_time_window_view_decoupling_20260628"
FORBIDDEN_CALLS = {
...
REQUIRED_VIEW_TOKENS = {
    "ui/timeline/timeline_widget.py": {
        "TimelineWidget.fit_to_view": (
            ...
            "self.global_canvas.update_viewport",
            "_sync_playhead_overlay",
        ),
        "TimelineWidget.schedule_fit_to_view": ("self.fit_to_view", "_manual_zoom_since_fit", "QTimer.singleShot"),
    },
    "ui/timeline/timeline_time_window.py": {
        "TimelineTimeWindowMixin.show_time_window_seconds": (
            "self.canvas.pps",
            "self.scroll.horizontalScrollBar().setValue",
            "self.global_canvas.update_viewport",
            "_sync_playhead_overlay",
        ),
        "TimelineTimeWindowMixin._apply_edit_window_seconds": (
            "show_time_window_seconds",
            "_manual_zoom_since_fit",
            "_begin_manual_scroll",
        ),
        "TimelineTimeWindowMixin.show_ten_second_edit_window": (
            "_editing_window_anchor_sec",
            "_apply_edit_window_seconds",
        ),
    },
}
...
def build_nle_time_window_view_decoupling_report() -> dict[str, Any]:
    method_rows = [
        _method_contract(ROOT / "ui" / "timeline" / "timeline_widget.py", "TimelineWidget", "fit_to_view"),
        _method_contract(ROOT / "ui" / "timeline" / "timeline_widget.py", "TimelineWidget", "schedule_fit_to_view"),
        _method_contract(
            ROOT / "ui" / "timeline" / "timeline_time_window.py",
            "TimelineTimeWindowMixin",
            "show_time_window_seconds",
        ),
        _method_contract(
            ROOT / "ui" / "timeline" / "timeline_time_window.py",
    ...
    return {
        ...
        "view_window_only_contract": True,
        ...
        "blocked_scope": [
            "time_window_fit_to_view_must_not_validate_or_rewrite_primary_subtitle_model",
            "time_window_fit_to_view_must_not_append_nle_operation_journal",
            "time_window_fit_to_view_must_not_save_project",
            "time_window_fit_to_view_must_not_change_ui_layout_labels_menus",
            "time_window_fit_to_view_must_not_enable_qml_or_gpu_default_surface",
        ],
    }


def write_nle_time_window_view_decoupling_report(output_dir: Path, report: dict[str, Any]) -> tuple[Path, Path]:
    ...
    lines = [
        "# NLE Time Window View Decoupling Audit",
        "",
        f"- Schema: `{report['schema']}`",
        f"- Ready: `{report['ready']}`",
        f"- Runtime change applied: `{report['runtime_change_applied']}`",
        f"- View-window-only contract: `{report['view_window_only_contract']}`",
...
def main(argv: list[str] | None = None) -> int:
    ...
    write_nle_time_window_view_decoupling_report(args.output_dir, report)
