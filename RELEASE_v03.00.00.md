<!--
Document-Version: 03.02.10
Phase: PHASE2
Last-Updated: 2026-05-01
Updated-By: Codex with 대표님
Previous-Content: v03.02.09 타임라인 하단 여백 제거 / 상단 마진 확대
This-Update: v03.02.10 터미널 로그 사이드바 통합 / `사이드바` 토글
Codex-Handoff: v03.02.10 코드 패치 기준 노트입니다. ACTION_ITEMS.md의 즉시 작업은 현재 `now: null`이고, 다음 코드 수정 버전은 v03.02.11입니다.
-->
# RELEASE v03.00.00
...
## 문서 / 액션아이템 정리
- v03.02.10에서 터미널 로그를 하단 패널에서 좌측 사이드바 하단으로 이동했습니다.
- 메인 하단 메뉴의 `터미널` 버튼명을 `사이드바`로 바꾸고, 해당 버튼이 좌측 프로젝트 사이드바 전체를 숨김/표시하도록 통합했습니다.
- 하단 메뉴바와 큐/러프컷 하단 패널은 오른쪽 메인 작업영역 안에만 배치해 `AI`부터 이어지는 아이콘이 사이드바 밖에 표시되도록 조정했습니다.
- 창 폭이 좁아질 때 하단 메뉴 아이콘/버튼 폭이 더 작게 접히도록 compact 기준과 아이콘 크기를 조정했습니다.
- `tests/test_sidebar_terminal_layout.py`를 추가해 터미널 로그의 사이드바 소속, `사이드바` 버튼명, 사이드바 토글 동작을 검증합니다.
- 삭제된 기능/함수/class는 없습니다. 터미널 로그 위치와 사이드바 토글 라우팅만 변경되었습니다.
- v03.02.09에서 타임라인 세그먼트 트랙 묶음 아래의 내부 하단 여백을 제거하고, 해당 높이만큼 `SEG_TOP_GAP`을 늘려 웨이브폼 아래 상단 마진을 확대했습니다.
- `CANVAS_H`는 `230`으로 유지하고 `SEG_BOT`을 `CANVAS_H`에 맞춰 하단에 붙도록 조정했습니다.
- `tests/test_timeline_layout_constants.py`를 갱신해 하단 여백 제거와 상단 마진 확대 상수를 검증합니다.
- 삭제된 기능/함수/class는 없습니다. 타임라인 레이아웃 상수만 조정되었습니다.
- v03.02.08에서 전체 타임라인 캔버스 높이는 유지한 채 자막 세그먼트/화자/음성감지/음성·무음 트랙 묶음을 아래쪽 글로벌 캔버스에 더 가깝게 이동했습니다.
- `CANVAS_H`는 기존 `230`을 유지하고, `SEG_TOP`에 상단 여유 공간을 추가해 웨이브폼 아래에 빈 공간이 남도록 조정했습니다.
- `tests/test_timeline_layout_constants.py`를 추가해 캔버스 높이 유지와 트랙 하단 정렬 상수를 검증합니다.
- 삭제된 기능/함수/class는 없습니다. 타임라인 레이아웃 상수와 하단 구분선 위치만 조정되었습니다.
- v03.02.07에서 타임라인 자막 세그먼트 텍스트가 `음성`이면 초록 계열, `무음`이면 주황 계열로 표시되도록 구분했습니다.
- 선택/hover 상태에서도 `음성`/`무음`의 기본 채움색과 텍스트 색을 유지해 한 화면에 섞여 있을 때 빠르게 구분됩니다.
- `tests/test_timeline_segment_colors.py`를 추가해 `음성`/`무음` 라벨 판별과 팔레트 분리를 검증합니다.
- 삭제된 기능/함수/class는 없습니다. 세그먼트 렌더링 색상 판별 helper만 추가되었습니다.
- v03.02.06에서 자막 에디터 선택/커서 이동, 타임라인 세그먼트 클릭, STT 블록 선택 시 플레이해드가 선택 세그먼트 시작 시간으로 함께 이동하도록 보강했습니다.
- 긴 영상에서 글로벌 캔버스 화면 맞춤이 5px/sec 기본 줌에 걸려 전체 맞춤까지 축소되지 않던 문제를 수정했습니다.
- 화면 맞춤 후 scroll 위치와 글로벌 캔버스 viewport를 즉시 전체 범위로 초기화해 확대 상태가 남지 않게 했습니다.
- `tests/test_timeline_playhead_fit.py`를 추가해 자막 선택 플레이해드 동기화와 긴 타임라인 fit 동작을 검증합니다.
- 삭제된 기능/함수/class는 없습니다. 선택 동기화와 fit 계산만 보강되었습니다.
- v03.02.05에서 iCloud/NAS 자동 처리 상태 문구를 `대기 : 영상 00개 / 음성 00개`, `완료 : 영상 00개 / 음성 00개` 형식으로 통일했습니다.
- 완료 상태에서 기존 `✅ 작업완료` 이모지/문구를 제거했습니다.
- NAS 자동 처리 상태도 폴더 개수 대신 영상/음성 파일 개수 기준으로 표시합니다.
- `tests/test_auto_status_labels.py`를 추가해 iCloud/NAS 상태 문구와 완료 카운트를 검증합니다.
- 삭제된 기능/함수/class는 없습니다. 자동 처리 카드 상태 문구와 카운트 기준만 변경되었습니다.
- v03.02.04에서 타임라인 화자 라벨 resolver를 추가해 `speaker`가 없거나 `SPEAKER_00`으로 들어온 세그먼트도 현재 `spk1_name`을 표시하도록 했습니다.
- 자막 생성 시작 직전에 editor settings를 최신 `user_settings.json`과 동기화하고 화자 strip/timeline을 갱신합니다.
- `ui/timeline/timeline_paint.py`의 `홍길동` fallback 표시를 현재 화자 1 이름 fallback으로 교체했습니다.
- `tests/test_timeline_speaker_labels.py`를 추가해 `소설가유모씨` 같은 사용자 화자명이 타임라인에 반영되는지 검증합니다.
- 삭제된 기능/함수/class는 없습니다. 화자명 표시 resolver와 생성 전 설정 동기화만 추가되었습니다.
- v03.02.03에서 단일 파일 파이프라인의 `optimizer` 스레드 최상단 예외를 앱 로그에 남기도록 보강했습니다.
- `core/engine/subtitle_engine.py`의 숫자 설정 파싱을 안전화해 `llm_threads`, `llm_workers`, 자막 길이/CPS/간격 설정이 빈 문자열 또는 비정상 값이어도 기본값으로 복구되게 했습니다.
- `tests/test_subtitle_engine_settings.py`를 추가해 설정값 fallback/default 동작을 검증합니다.
- 삭제된 기능/함수/class는 없습니다. 예외 로깅과 설정 파싱 안정화만 추가되었습니다.
- v03.02.02에서 AI 설정창을 `빠른 설정`, `에디터 LLM`, `러프컷 LLM`, `AI` 탭으로 정리했습니다.
- `빠른 설정` 탭에는 자막 정확도 프리셋과 오디오 프리셋을 배치했습니다.
- `에디터 LLM` 탭에는 LLM 모델/API Key/모델 관리, 기존 시스템 설정의 LLM 프롬프트, 자막 묶음 단위, `에디터 LLM 처리 스레드`를 배치하고 엔진의 기존 `llm_workers` 키와 호환되게 저장/로드를 연결했습니다.
- `러프컷 LLM` 탭에는 러프컷 LLM override/provider/model/API key mode/temperature/row/prompt와 `러프컷 LLM 처리 스레드`를 배치했습니다.
- `AI` 탭에는 Whisper 모델, 음성 처리 AI, VAD, 기존 시스템 설정의 자막 품질 검사/자동 교정/memory/자동 적용 기준/재검사 buffer를 이동했습니다.
- 시스템 상세 설정창에서는 중복되는 `LLM 프롬프트`, `자막 품질`, `LLM 처리 스레드` 항목을 제거했습니다.
- `roughcut_llm_threads` 설정을 추가하고 `core/roughcut/roughcut_llm_config.py` resolver에 반영했습니다.
- `tests/test_roughcut_ui_v2.py`와 `tests/test_roughcut_models_v2.py`를 확장해 AI 설정 탭 구성, 시스템 설정 중복 제거, editor/roughcut LLM thread 저장을 검증합니다.
- 삭제된 기능/함수/class는 없습니다. 기존 설정 키 `user_prompt`, `llm_threads`, 자막 품질 설정은 유지됩니다.
- v03.02.01에서 PHASE2-D PAGE3B 러프컷 전용 LLM 설정/프롬프트 분리 1차를 반영했습니다.
- `config.DEFAULT_ROUGHCUT_SETTINGS`에 PAGE3B 요구 설정값을 보강하고 `APP_VERSION`을 `03.02.01`로 갱신했습니다.
- `core/roughcut/roughcut_llm_config.py`를 추가해 기존 자막 생성 LLM 설정 상속과 러프컷 전용 provider/model/prompt override를 분리했습니다.
- `core/roughcut/roughcut_prompts.py`에 PAGE3B 기본 러프컷 프롬프트를 추가하고 LLM action prompt에 포함되도록 보강했습니다.
- AI 설정창에 러프컷 LLM 사용 여부, override, provider/model, API Key mode, temperature, row 처리 단위, 프롬프트 편집/기본값 복원 UI를 추가했습니다.
- API Key 원문은 기존 보안 저장소 경로를 유지하고 settings JSON에는 저장하지 않습니다.
- `tests/test_roughcut_models_v2.py`와 `tests/test_roughcut_ui_v2.py`를 확장해 설정 상속/override, 기본 프롬프트 포함, 설정창 저장 수집을 검증합니다.
- 삭제된 기능/함수/class는 없습니다. 기존 자막 생성 LLM 설정, 러프컷 local fallback, legacy QTableWidget fallback은 유지됩니다.
