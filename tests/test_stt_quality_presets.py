...
class STTQualityPresetTests(unittest.TestCase):
    ...
    def test_apply_fast_preset_uses_whisper_only_and_speed_settings(self):
        ...
        self.assertTrue(applied["stt_low_score_recheck_enabled"])
        self.assertEqual(applied["stt_low_score_recheck_threshold"], 64)
        self.assertEqual(applied["stt_low_score_recheck_max_segments"], 8)
        self.assertAlmostEqual(applied["stt_low_score_recheck_max_audio_sec"], 45.0)
    ...
    def test_precise_preset_is_stricter_than_balanced(self):
        ...
        self.assertTrue(balanced["stt_low_score_recheck_enabled"])
        self.assertEqual(balanced["stt_low_score_recheck_threshold"], 74)
        self.assertEqual(balanced["stt_low_score_recheck_max_segments"], 18)
        self.assertFalse(balanced["stt_word_timestamps_precision_enabled"])
        self.assertTrue(balanced["audio_chunk_routing_enabled"])
        ...
        self.assertTrue(precise["stt_word_timestamps_precision_enabled"])
        self.assertEqual(precise["stt_word_timestamps_precision_max_segments"], 48)
        ...
        self.assertFalse(precise["audio_chunk_routing_enabled"])
        self.assertFalse(precise["audio_chunk_route_vad_enabled"])
        self.assertTrue(precise["audio_preset_auto_benchmark_locked"])
        self.assertEqual(precise["audio_chunk_profile_sec"], 24.0)
        self.assertEqual(precise["audio_chunk_route_profile_samples"], 3)
        self.assertTrue(precise["audio_chunk_route_preview_enabled"])
        self.assertTrue(precise["stt_low_score_recheck_enabled"])
        self.assertEqual(precise["stt_low_score_recheck_threshold"], 78)
        self.assertEqual(precise["stt_low_score_recheck_max_segments"], 24)
        self.assertAlmostEqual(precise["stt_low_score_recheck_max_audio_sec"], 110.0)
        self.assertAlmostEqual(precise["vad_threshold"], 0.36)
    ...
    def test_recommended_defaults_ignore_old_saved_policy_but_keep_user_choices(self):
        ...
        self.assertTrue(applied["stt_ensemble_enabled"])
        self.assertEqual(applied["stt_word_timestamps_precision_max_segments"], 48)
        self.assertTrue(applied["stt_word_timestamps_precision_enabled"])
