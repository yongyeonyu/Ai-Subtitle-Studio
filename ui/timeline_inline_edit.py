...
TimelineCanvas의 인라인 편집 + 음성인식 + IME 분리
"""
import threading
...
import config
from ui.timeline_constants import SEG_TOP, SEG_BOT, HANDLE_R
...
class TimelineInlineEditMixin:
    ...
    def _commit_inline_edit(self):
        ...
        for s in self.segments:
            if s.get("line") == line: s["text"] = self._edit_text; break
        self.sig_inline_text_changed.emit(line, safe_for_editor)
        if hasattr(self, "_pending_split_sec"): del self._pending_split_sec
        self._end_inline_edit()
    ...
    def _handle_edit_key(self, ev):
        ...
        def _row_col(t, c):
            ...
            for i, ln in enumerate(lines):
                ...
                r = len(lines) - 1; col = max(0, col)
            return r, col, lines

        if mods & (Qt.KeyboardModifier.MetaModifier | Qt.KeyboardModifier.ControlModifier):
            if key == Qt.Key.Key_Left:
                self._edit_cursor = 0; self._cursor_vis = True; self.update(); return
            elif key == Qt.Key.Key_Right:
                self._edit_cursor = len(self._edit_text); self._cursor_vis = True; self.update(); return
        ...
        if key in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
            if ev.modifiers() & Qt.KeyboardModifier.ShiftModifier:
                ...
                if hasattr(self, "_pending_split_sec"):
                    ...
                    self.sig_split_request.emit(int(self._edit_line), float(self._pending_split_sec), int(self._edit_cursor))
            ...
            self._edit_text = self._edit_orig; self._end_inline_edit(); return
        ...
        elif key == Qt.Key.Key_Left: self._edit_cursor = max(0, cur - 1)
        elif key == Qt.Key.Key_Right: self._edit_cursor = min(len(self._edit_text), cur + 1)
        elif key == Qt.Key.Key_Up:
            r, col, lines = _row_col(text, cur)
            if r > 0: self._edit_cursor = sum(len(l) + 1 for l in lines[:r - 1]) + min(col, len(lines[r - 1]))
            else: self._edit_cursor = 0
        elif key == Qt.Key.Key_Down:
            r, col, lines = _row_col(text, cur)
            if r < len(lines) - 1: self._edit_cursor = sum(len(l) + 1 for l in lines[:r + 1]) + min(col, len(lines[r + 1]))
    ...
    def _cancel_inline_edit(self):
        if not self._edit_active: return
        for s in self.segments:
            if s.get("line") == self._edit_line: s["text"] = self._edit_orig; break
    ...
    def inputMethodQuery(self, query):
        ...
        if self._edit_active and query == _Qt.InputMethodQuery.ImCursorRectangle:
            ...
            if seg: return QRect(self._x(seg["start"]), SEG_TOP + 5, 1, 20)
    ...
    def _show_mic_menu(self, gpos):
        ...
        if self._is_listening:
            act = menu.addAction("🔴 음성인식 중지"); act.triggered.connect(self._stop_listening)
        else:
            act = menu.addAction("🎤 음성으로 입력"); act.triggered.connect(self._start_listening)
        menu.exec(gpos)

    def _start_listening(self):
        if self._is_listening: return
        self._is_listening = True; self.update()
        def _listen():
            try:
                ...
                if text and text.strip(): self.sig_speech_result.emit(text.strip())
    ...
    def _stop_listening(self):
        self._is_listening = False; self.update()
