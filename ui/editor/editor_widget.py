...
class EditorWidget(
    ...
    sig_subtitle_post_llm_finished = pyqtSignal(str, object, object)
    sig_subtitle_post_llm_failed = pyqtSignal(str, str)
    ...
    def __init__(
        ...
        self.sig_subtitle_post_llm_finished.connect(self._on_subtitle_post_llm_finished)
        self.sig_subtitle_post_llm_failed.connect(self._on_subtitle_post_llm_failed)
    ...
    def _build_ui(self):
        ...
        if hasattr(self.timeline, 'subtitle_text_height_requested'): self.timeline.subtitle_text_height_requested.connect(lambda *args: self._show_export_dialog(initial_tab="text_height"))
        if hasattr(self.timeline, 'subtitle_transparent_mov_requested'): self.timeline.subtitle_transparent_mov_requested.connect(self._show_export_dialog)
        if hasattr(self.timeline, 'subtitle_overlay_video_requested'): self.timeline.subtitle_overlay_video_requested.connect(lambda *args: self._show_export_dialog(output_mode="subtitle_with_video"))
        ...
        if hasattr(self.timeline, 'subtitle_spellcheck_requested'): self.timeline.subtitle_spellcheck_requested.connect(self._handle_global_subtitle_spellcheck_request)
        if hasattr(self.timeline, 'subtitle_translate_english_requested'): self.timeline.subtitle_translate_english_requested.connect(self._handle_global_subtitle_translate_english_request)
    ...
    def _open_subtitle_output_dialog(self) -> None:
        self._show_export_dialog()

    def _subtitle_overlay_default_output_path(self) -> str:
        media_path = str(getattr(self, "media_path", "") or "")
        if not media_path:
            return ""
        base = os.path.splitext(os.path.basename(media_path))[0]
        return os.path.join(os.path.dirname(media_path), f"{base}_자막입힘.mp4")

    def _export_subtitle_overlay_video_gpu(self) -> None:
        media_path = str(getattr(self, "media_path", "") or "")
        if not media_path or not os.path.exists(media_path):
            QMessageBox.warning(self, "자막 출력", "원본 영상 파일을 찾을 수 없습니다.")
            return
        if bool(getattr(self, "_subtitle_overlay_export_running", False)):
            QMessageBox.information(self, "자막 출력", "이미 영상+자막 오버레이 출력이 진행 중입니다.")
            return
        try:
            segs = self._get_current_segments(force_rebuild=True)
        except TypeError:
            segs = self._get_current_segments()
        segs = [dict(seg) for seg in list(segs or []) if isinstance(seg, dict)]
        if not any(str(seg.get("text", "") or "").strip() and not seg.get("is_gap") for seg in segs):
            QMessageBox.information(self, "자막 출력", "출력할 자막 세그먼트가 없습니다.")
            return
        output_path = self._subtitle_overlay_default_output_path()
        self._subtitle_overlay_export_running = True
        get_logger().log(f"🎥 영상+자막 오버레이 출력(GPU) 예약: {os.path.basename(output_path)}")

        def worker() -> None:
            import shutil
            import tempfile

            tmp_dir = tempfile.mkdtemp(prefix="ai_subtitle_overlay_")
            try:
                ...
                tmp_srt = os.path.join(tmp_dir, "current_subtitles.srt")
                save_srt(segs, tmp_srt, fps=getattr(self, "video_fps", 30.0), write_backup=False)
                ...
                if ok:
                    ...
                    get_logger().log("❌ 영상+자막 오버레이 출력 실패")
            except Exception as exc:
                get_logger().log(f"❌ 영상+자막 오버레이 출력 오류: {exc}")
            finally:
                shutil.rmtree(tmp_dir, ignore_errors=True)
                self._subtitle_overlay_export_running = False

        import threading

        threading.Thread(target=worker, daemon=True, name="subtitle-overlay-video-gpu-export").start()
    ...
    def _build_buttons(self) -> QWidget:
        ...
        self._bot_btns = [
            ...
            (self.btn_exp,   "🎥 자막출력", "출력", self._open_subtitle_output_dialog),
        ]
