...
class EditorWidget(EditorActionsMixin, EditorPipelineMixin, EditorSegmentsMixin, EditorTimelineVideoMixin, QWidget):
    ...
    def _apply_active_context(self, ctx: dict, autoplay: bool = False, show_thumbnail: bool = True):
        if not ctx or not hasattr(self, 'video_player'):
            return
        # Lazy-init EndOfMedia callback for multiclip advance (D fix)
        ...
        if hasattr(self, 'timeline') and hasattr(self.timeline, 'canvas'):
            ...
            # Invalidate playback caches for context switch
    ...
    def _on_end_of_clip(self):
        """EndOfMedia: multiclip -> advance to next clip (D fix)"""
        boxes = list(getattr(self.timeline.canvas, '_multiclip_boxes', []) or []) if hasattr(self, 'timeline') else []
        if not boxes:
            return  # single clip: normal end
