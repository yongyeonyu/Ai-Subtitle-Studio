...
class RoughcutStateMixin:
    ...
    def _topicless_placeholder_result_from_project(self):
        """
        STT/자막 segments가 아직 없더라도 project에 저장된
        cut_boundary topicless placeholder를 RoughCutResult로 변환한다.
        """
        project_path = self._project_path()
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            major_id = str(row.get("major_id") or f"cut_topicless_{idx:03d}")
            segment_id = str(row.get("segment_id") or row.get("id") or major_id)
            chapter_id = str(row.get("chapter_id") or f"{segment_id}_chapter")
            minor_id = str(row.get("minor_id") or f"{segment_id}_minor")
...
# === ROUGHCUT FRAME SYNCED TOPICLESS START ===
...
def _frame_synced_topicless_placeholder_result_from_project(self):
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        if not major_id or major_id.startswith("cut_topicless_middle"):
            major_id = _pipeline_topicless_middle_label(idx) if "_pipeline_topicless_middle_label" in globals() else chr(64 + min(idx, 26))
...
RoughcutStateMixin._topicless_placeholder_result_from_project = _frame_synced_topicless_placeholder_result_from_project

# === ROUGHCUT FRAME SYNCED TOPICLESS END ===
