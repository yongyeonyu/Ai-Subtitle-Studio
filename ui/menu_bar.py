...
class GlobalMenuBar(QWidget):
    def __init__(self, main_window):
        ...
        for text, icon, slot, color in [
            ...
            left.addWidget(self._small_button(text, icon, slot, color))
        ...
        for btn in (self.btn_start, self.btn_save):
        ...
        for text, icon, slot, kind in [
            ("자동설정", "settings", self._open_auto_settings, "toolbar"),
            ("캐쉬삭제", "trash", self._clear_cache, "toolbar"),
        ]:
            right.addWidget(self._wide_button(text, icon, slot, kind=kind))
    ...
    def _wide_button(self, text, icon_name, slot, *, kind="toolbar"):
        btn = QToolButton()
        btn.setText(text.replace("자동설정", "자동\n설정").replace("캐쉬삭제", "캐쉬\n삭제"))
    ...
    def refresh(self):
        ...
        for btn in self._tool_buttons:
            if compact:
                ...
                if btn in (self.btn_start, self.btn_save):
                    btn.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonTextUnderIcon)
                    btn.setMinimumSize(92, 58)
                    btn.setMaximumWidth(16777215)
                elif btn in (self.btn_auto_start, self.btn_log):
                    btn.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonTextUnderIcon)
                    btn.setMinimumSize(72, 52)
                    btn.setMaximumWidth(16777215)
                else:
