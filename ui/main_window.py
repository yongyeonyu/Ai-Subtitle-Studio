...
ui/main_window.py
[v01.00.04 ?˜ì •?¬í•­]
- ë¶€?œëª© "?Œì„¤ê°€? ëª¨???„ìš© ?ë§‰ ?ë™ ?ì„±ê¸? ?ìŠ¤???œê±°
- ìµœê·¼ ?´ë” ë²„íŠ¼: _recent_buttons ì¶”ì , ë¡œê·¸ ?´ë¦´ ??1ê°œë§Œ ?œì‹œ
- closeEvent: ???„ë‹ˆ??ì·¨ì†Œ StandardButton ë°©ì‹?¼ë¡œ editor_pipelineê³??µì¼
- _init_editor: ì¤‘ë³µ ?œê·¸???°ê²° ì½”ë“œ ?œê±°
"""
import os, json, re
...
class MainWindow(QMainWindow):
    ...
    # ?’¡ [? ê·œ ì¶”ê?] ?ë™ ?œì‘ ?„ìš© ë¬´ì „ê¸?Signal)ë¥?ë§Œë“­?ˆë‹¤.
    _sig_auto_start_pipeline = pyqtSignal(list)

    def __init__(self):
        super().__init__()
        self.setWindowTitle("?¬ AI PD Studio - ?Œì„¤ê°€? ëª¨??)
        ...
        # [MainWindow.__init__ ?´ë? ?˜ì •]
        self._is_icloud_auto_mode = get_icloud_auto_detect() # iCloud ?„ìš©
        self._is_nas_auto_mode = get_nas_auto_detect()       # NAS ?„ìš©
        ...
        # ?’¡ [?€ì¹˜ë… ?¥ì°© ë¶€ë¶??˜ì •]
        # ??ë²ˆì§¸ ?¸ìë¡?self._is_app_busyë¥??˜ê²¨ì£¼ì–´??'?ˆì¹˜ ë³´ê¸°' ê¸°ëŠ¥???‘ë™?©ë‹ˆ??
    ...
    def _on_files_detected(self, files_list):
        get_logger().log(f"?? ?ë™ ì²˜ë¦¬ ??ì§„ì…: {len(files_list)}ê°??Œì¼")
        # ?’¡ [?µì‹¬] ?€?´ë¨¸(QTimer) ?€???ˆì „??ë¬´ì „ê¸?Signal)ë¥??´ì„œ ë©”ì¸ ?¤ë ˆ?œë¡œ ?˜ê¹?ˆë‹¤!
    ...
    def _do_auto_start_pipeline(self, files_list):
        if self.backend:
            ...
            # 2. ?’¡ [?˜ì •] ?€?´ë¨¸ ?†ì´ ì¦‰ì‹œ ?íƒœ ë°˜ì˜ (?ë””?°ê? ?´ë? ? ìˆ??ê²½ìš°)
            if self._editor_widget and hasattr(self._editor_widget, 'update_status'):
                self._editor_widget.update_status("?? AI ?”ì§„???œì‘?˜ì—ˆ?µë‹ˆ?? (?ë™ ê°ì?)")

    def _is_app_busy(self) -> bool:
        """?„ì¬ ?ë””?°ê? ?´ë ¤?ˆê±°???‘ì—… ì¤‘ì¸ì§€ ?•ì¸?©ë‹ˆ??"""
    ...
    def mark_cloud_file_done(self, filepath: str):
        """backend ì²˜ë¦¬ ?„ë£Œ ??CloudSyncManager _in_flight ë°?tracker ?íƒœ ?´ì œ"""
    ...
    def _build_log_panel(self) -> QWidget:
        ...
        self._log_toggle_btn = QLabel("???°ë???ë¡œê·¸ ë³´ê¸°"); self._log_toggle_btn.setStyleSheet(f"color: {config.FG2}; font-size: 11px; font-weight: bold;")
        ...
        self.queue_header_lbl = QLabel("?“‹ ì²˜ë¦¬???Œì¼ ë¦¬ìŠ¤??)
        ...
        self.queue_table.setHorizontalHeaderLabels(["  ?íƒœ  ", "  ?Œì¼ëª? ", "  ?ìƒ?•ë³´  ", "  ?ìƒê¸¸ì´  ", "  ?ˆìƒ?œê°„  "])
        ...
        if self._log_visible: self._log_content.show(); self._log_toggle_btn.setText("???°ë???ë¡œê·¸ ?¨ê¸°ê¸?)
        else: self._log_content.hide(); self._log_toggle_btn.setText("???°ë???ë¡œê·¸ ë³´ê¸°")
        self._queue_anim_frames = ["?“‘", "?“„", "?“ƒ", "?“"]; self._queue_anim_idx = 0
    ...
    # [ui/main_window.py] _get_icloud_files ?¨ìˆ˜ êµì²´
    def _get_icloud_files(self) -> tuple[list, str, str]:
        ...
        if not path or not os.path.exists(path):
            ...
            if not os.path.exists(path): return [], "ê²½ë¡œ ?†ìŒ", ""
        
        v_exts = {'.mov', '.mp4', '.m4v', '.MOV', '.MP4', '.M4V', '.lrf', '.LRF'}
        a_exts = {'.wav', '.m4a', '.mp3', '.aac', '.m2a'}
        v_count, a_count = 0, 0
        comp_v_count, comp_a_count = 0, 0 # ?„ë£Œ???Œì¼ ì¹´ìš´??
        files = []
        ...
        try:
            ...
            count_str = f"?€ê¸?: ?ìƒ {v_count:02d}ê°?/ ?Œì„± {a_count:02d}ê°?
            comp_str = f"???‘ì—…?„ë£Œ : ?ìƒ {comp_v_count:02d}ê°?/ ?Œì„± {comp_a_count:02d}ê°?
    ...
    def _icloud_btn(self, text, file_data, default_cmd, is_nas=False, subtitle="", comp_title=""):
        ...
        # ?’¡ [?µì‹¬] ?ë™ ê°ì? ?¤ì •(self._is_auto_mode)???°ë¼ ê¸€?ìƒ‰ ê²°ì •
        if is_nas:
        ...
        # ?’¡ [? ê·œ ì¶”ê?] ?‘ì—… ?„ë£Œ ?ìŠ¤?¸ë? ì´ˆë¡?‰ìœ¼ë¡??ˆì˜ê²??„ì›Œì¤ë‹ˆ??
        if comp_title:
        ...
        if not file_data:
            empty_lbl = QLabel("?€ê¸?ì¤‘ì¸ ?Œì¼???†ìŠµ?ˆë‹¤."); empty_lbl.setStyleSheet(f"color: {config.FG2}; font-size: 11px; border: none;"); preview_layout.addWidget(empty_lbl)
        else:
    ...
    def _animate_queue_status(self):
        ...
        for i in range(self.queue_table.rowCount()):
            ...
            if item:
                txt = item.text()
                # ?’¡ ?íƒœ???°ë¼ ? ë‹ˆë©”ì´???“„)???¤ë¥´ê²?ë¶™ì—¬ì¤ë‹ˆ??
                if "?ë§‰ ?ì„± ì¤? in txt:
                    item.setText(f"{self._queue_anim_frames[self._queue_anim_idx]} ?ë§‰ ?ì„± ì¤?)
                elif "?ë§‰?ìƒì¶œë ¥" in txt or "?ìƒì¶œë ¥" in txt:
                    item.setText(f"{self._queue_anim_frames[self._queue_anim_idx]} ?ë§‰?ìƒì¶œë ¥(mov)")
    ...
    def _build_home_content(self):
        ...
        # 1. ?Œì¼/?´ë” ? íƒ ë©”ë‰´??
        standard_menus = [
            ("?“‚ ?Œì¼ ? íƒ", "?ìƒ/?Œì„±/srt ?Œì¼ ì§ì ‘ ? íƒ", self.select_files), 
            ("?“ ?´ë” ? íƒ", "?¤ì´?¼ë¡œê·¸ì—???ìƒ ?Œì¼ ?¼ê´„/ë¶€ë¶?? íƒ", self.select_folder), 
            ("?‚ï¸ cut ?¸ì§‘ ?„ìš°ë¯?, "ê°œë°œ ì¤?, self._dummy_action)
        ]
        for t, d, s in standard_menus: 
            layout.addWidget(self._btn(t, d, s))
        
        # 2. ?’¡ [?µì‹¬] iCloud ?ë™ ì²˜ë¦¬ (ì¤‘ë³µ ?œê±° ë°?ê°œìˆ˜ ?œì‹œ ?ìš©)
        # ?’¡ [?˜ì • ?? comp_str??ì¶”ê??´ì„œ 3ê°œë? ë°›ì•„???©ë‹ˆ??
        icloud_files, count_str, comp_str = self._get_icloud_files()
        layout.addWidget(self._icloud_btn("?ï¸ iCloud ?ë™ ì²˜ë¦¬", icloud_files, self.start_icloud_sync, subtitle=count_str, comp_title=comp_str))
        
        # 3. NAS ?ë™ì²˜ë¦¬ ë²„íŠ¼
        nas_folders = self._get_nas_folders()
        layout.addWidget(self._icloud_btn("?—„ï¸?NAS ?ë™ì²˜ë¦¬", nas_folders, self._open_nas_root, is_nas=True))
        ...
        if valid_folders:
            ...
            recent_lbl = QLabel("?“‚ ìµœê·¼ ?´ë”"); recent_lbl.setStyleSheet(f"color: {config.FG}; font-size: 14px; font-weight: bold; border: none; background: transparent;"); recent_layout.addWidget(recent_lbl)
            desc_lbl = QLabel("ìµœê·¼???‘ì—…???´ë” ë¦¬ìŠ¤??); desc_lbl.setStyleSheet(f"color: {config.FG2}; font-size: 11px; border: none; background: transparent;"); recent_layout.addWidget(desc_lbl)

            # [v01.00.04] ë¡œê·¸ ?¨ë„???´ë ¤ ?ˆìœ¼ë©?1ê°œë§Œ, ?«í? ?ˆìœ¼ë©?ìµœë? 3ê°??œì‹œ
        ...
        # 5. ?˜ë‹¨ ë°?(ë²„ì „ ?•ë³´ ë°??¤ì •/ì¢…ë£Œ ë²„íŠ¼)
        bottom_bar = QHBoxLayout()
        version_lbl = QLabel("v0.1.0 (Build 01.00.00)"); version_lbl.setStyleSheet(f"color: {config.FG2}; font-size: 11px;")
        btn_settings = QPushButton("?™ï¸ ê²½ë¡œ?¤ì •"); btn_settings.setStyleSheet(f"background: {config.BG3}; color: {config.FG}; border: none; padding: 6px 12px; border-radius: 4px;"); btn_settings.clicked.connect(self._show_path_settings)
        btn_clear_cache = QPushButton("?—‘ï¸?ìºì‰¬?? œ"); btn_clear_cache.setStyleSheet(f"background: {config.BG3}; color: {config.FG}; border: none; padding: 6px 12px; border-radius: 4px;"); btn_clear_cache.clicked.connect(self._clear_cache)
        btn_exit = QPushButton("??ì¢…ë£Œ"); btn_exit.setStyleSheet(f"background: #882222; color: #FFF; font-weight: bold; border: none; padding: 6px 12px; border-radius: 4px;"); btn_exit.clicked.connect(self._quick_exit)
    ...
    # [ui/main_window.py] init_queue_list ë°?update_queue_status êµì²´
    def init_queue_list(self, files: list):
        ...
        # ?’¡ [ë¬¸êµ¬ êµì²´] ?œì‘ ?„ì—???Œì „?˜ê²Œ '?€ê¸?ì¤??¼ë¡œ ?œì‹œ?©ë‹ˆ??
        self.queue_header_lbl.setText(f"?“‹ ì²˜ë¦¬???Œì¼ ë¦¬ìŠ¤??(1 / {len(files)} ?€ê¸?ì¤? - 0% ?„ë£Œ [?±ï¸ 00:00 / 00:00]")        
        for i, f in enumerate(files):
            self.queue_table.insertRow(i)
            def make_item(text):
                it = QTableWidgetItem(text); it.setTextAlignment(Qt.AlignmentFlag.AlignCenter); return it
                
            self.queue_table.setItem(i, 0, make_item("???€ê¸?ì¤?))
            self.queue_table.setItem(i, 1, QTableWidgetItem(os.path.basename(f)))
            self.queue_table.setItem(i, 2, make_item("ë¶„ì„ ì¤?.."))
            self.queue_table.setItem(i, 3, make_item("-"))
            self.queue_table.setItem(i, 4, make_item("ê³„ì‚° ì¤?))
    ...
    def update_queue_status(self, idx: int, status: str, time_txt: str = "", info_txt: str = "", len_txt: str = ""):
        if idx < self.queue_table.rowCount():
            ...
            if status: 
                ...
                # ?’¡ "?ë§‰ ?ì„± ì¤????œì‘?˜ëŠ” ?œê°„???œê° ê¸°ë¡
                if "?ë§‰ ?ì„± ì¤? in status and idx not in self._file_start_times:
                    import time
    ...
    # [ui/main_window.py] _update_live_queue_header ?¨ìˆ˜ ?„ì²´ êµì²´
    def _update_live_queue_header(self):
        ...
        # ?’¡ [?ë‹¨ ?¤ë”] ?„ì²´ ?œê°„ ?©ê³„ ?œì‹œ (Total Elapsed / Total Expected)
        ...
        self.queue_header_lbl.setText(f"?“‹ ì²˜ë¦¬???Œì¼ ë¦¬ìŠ¤??({c} / {t} ì§„í–‰ ì¤? - {pct}% ?„ë£Œ   [?±ï¸ {fmt_mm_ss(elapsed_total)} / {fmt_mm_ss(expected_total)}]")

        # ?’¡ [???Œì´ë¸? ?„ì¬ ì§„í–‰ ì¤‘ì¸ ?Œì¼???œê°„ ?Œë”ë§?
        for i in range(self.queue_table.rowCount()):
            status_item = self.queue_table.item(i, 0)
            if status_item and "?ë§‰ ?ì„± ì¤? in status_item.text():
                # ?´ë‹¹ ?Œì¼???œì‘???œê° ê¸°ì? ê²½ê³¼ ?œê°„ ê³„ì‚°
    ...
    def _clear_cache(self):
        reply = QMessageBox.question(
            self, 'ìºì‰¬ ?? œ', 
            'output ?´ë” ?´ì˜ ?„ì‹œ ?Œì¼?¤ì„ ëª¨ë‘ ?? œ?˜ì‹œê² ìŠµ?ˆê¹Œ?\n(ì§„í–‰ ì¤‘ì¸ ?‘ì—…???†ì„ ?Œë§Œ ?¬ìš©?˜ì„¸??)', 
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            # ?’¡ [êµì •] ì¤‘ì²©??tryë¥??œê±°?˜ê³  ë¬¸ë²• êµ¬ì¡°ë¥??•ì„?€ë¡?ë°”ë¡œ?¡ì•˜?µë‹ˆ??
            try:
                # 1. ë¬¼ë¦¬???Œì¼ ?? œ
                ...
                # 3. ê°ì‹œ ?”ì›(CloudSyncManager)??ë¨¸ë¦¿??ë©”ëª¨ë¦???ë¹„ìš°ê¸?
                if hasattr(self, '_cloud_sync_manager'):
                    mgr = self._cloud_sync_manager
                    mgr._size_cache.clear()   # ?Œì¼ ?¬ê¸° ê¸°ì–µ ?? œ
                    mgr._in_flight.clear()    # ?„ì¬ ?‘ì—… ì¤?ëª©ë¡ ?? œ
                    if hasattr(mgr.tracker, '_data'):
                        mgr.tracker._data.clear() # ì¶”ì ê¸??´ë? ?°ì´???? œ
                
                get_logger().log("?§¹ ëª¨ë“  ìºì‹œ?€ ê°ì‹œ??ë©”ëª¨ë¦¬ê? ë¦¬ì…‹?˜ì—ˆ?µë‹ˆ??")
                QMessageBox.information(self, "?„ë£Œ", "?—‘ï¸?ìºì‰¬ ë°??ë™ ëª¨ë“œ ê¸°ë¡??ì´ˆê¸°?”ë˜?ˆìŠµ?ˆë‹¤.")
                self.show_home() # ?”ë©´ ê°±ì‹ 
                
            except Exception as e:
                get_logger().log(f"??ìºì‹œ ?? œ ì¤??¤ë¥˜ ë°œìƒ: {e}")
                QMessageBox.warning(self, "?¤ë¥˜", f"?? œ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤: {e}")

    # ?’¡ [?µì‹¬] ?„ë£Œ (100%) ???€?´ë¨¸ ì¤‘ë‹¨ ë°?ìµœì¢… ?¼ë²¨ ?•ì •
    def update_queue_header(self, current: int, total: int, pct: int, eta_str: str = ""):
        ...
        if pct == 100:
            ...
            self.queue_header_lbl.setText(f"?“‹ ì²˜ë¦¬???Œì¼ ë¦¬ìŠ¤??({total} / {total} ?„ë£Œ) - 100% ?„ë£Œ")
    ...
    def _connect_signals(self):
        ...
        # ?’¡ [? ê·œ ì¶”ê?] ë¬´ì „ê¸°ê? ?¸ë¦¬ë©?_do_auto_start_pipeline???¤í–‰?˜ë„ë¡??°ê²°?©ë‹ˆ??
    ...
    def _open_recent(self, folder):
        if not os.path.exists(folder):
            if not ensure_nas_mounted(folder):
                QMessageBox.warning(self, "?¤ë¥˜", f"?´ë”ë¥?ì°¾ì„ ???†ìŠµ?ˆë‹¤:\n{folder}")
                return
    ...
    def _toggle_log(self):
        ...
        # [v01.00.04] ë¡œê·¸ ?´ë¦´ ??ìµœê·¼ ?´ë” 1ê°œë§Œ, ?«í ??ìµœë? 3ê°??œì‹œ
    ...
    def show_home(self):
        ...
        # ?’¡ [?µì‹¬ ë²„ê·¸ ?˜ì •] ?Œë¦¼ì°½ì´ ? ìˆ?????Œì´??GCê°€ ë©”ëª¨ë¦¬ë? 
        # ê°•ì œ ?? œ?˜ì—¬ ë°œìƒ?˜ëŠ” C++ ì¶©ëŒ(SIGABRT) ë°©ì?
        if self._editor_widget:
            ...
            if len(self._trash_bin) > 3:
                self._trash_bin.pop(0) # ?¤ë˜??ë©”ëª¨ë¦¬ë????ˆì „?˜ê²Œ ë¹„ìš°ê¸?
    ...
    def _do_update_status(self, c_idx: int, t_total: int):
        if self._editor_widget:
            if hasattr(self._editor_widget, 'update_progress'): self._editor_widget.update_progress(c_idx, t_total)
                ...
                if c_idx < t_total: self._editor_widget.update_status(f"??ì²˜ë¦¬ì¤?.. ({c_idx:02d}/{t_total:02d}ê°?ì²?¬)")
                else: self._editor_widget.update_status("???ì„± ?„ë£Œ! ë§ˆìŒê»??¸ì§‘?˜ì„¸??")
    ...
    def _open_nas_root(self):
        ...
        if not nas_url:
            QMessageBox.warning(self, "?¤ë¥˜", "NAS ê²½ë¡œê°€ ?¤ì •?˜ì? ?Šì•˜?µë‹ˆ?? ?¤ì • ë©”ë‰´ë¥??•ì¸?˜ì„¸??")
            return
        if not ensure_nas_mounted(nas_url):
            QMessageBox.warning(self, "?¤ë¥˜", "NAS ë§ˆìš´?¸ì— ?¤íŒ¨?ˆìŠµ?ˆë‹¤.")
            return
    ...
    def select_folder(self):
        folder = QFileDialog.getExistingDirectory(self, "?´ë” ? íƒ", get_last_folder() or os.path.expanduser("~"))
    ...
    def select_files(self):
        paths, _ = QFileDialog.getOpenFileNames(self, "?Œì¼ ? íƒ", get_last_folder() or os.path.expanduser("~"), "Media/SRT Files (*.mp4 *.mov *.MOV *.MP4 *.wav *.m4a *.m2a *.mp3 *.aac *.srt)")
        if not paths: return
    ...
    def open_editor_directly(self):
        path, _ = QFileDialog.getOpenFileName(self, "SRT ?Œì¼ ? íƒ", get_last_folder() or os.path.expanduser("~"), "SRT Files (*.srt)")
        if path: 
    ...
    def _show_path_settings(self):
        """[ê²½ë¡œ?¤ì •] ë²„íŠ¼ ?´ë¦­ ???˜í??˜ëŠ” ?¤ì • ?¤ì´?¼ë¡œê·?""
        dlg = QDialog(self)
        dlg.setWindowTitle("ê²½ë¡œ?¤ì •")
        dlg.setMinimumWidth(450)
        dlg.setStyleSheet(f"background-color: {config.BG}; color: {config.FG};")
        
        layout = QVBoxLayout(dlg)
        
        layout.addWidget(QLabel("NAS ë£¨íŠ¸ ê²½ë¡œ (?? smb://DDuDDu_NAS._smb._tcp.local/video):"))
        ...
        layout.addWidget(QLabel("iCloud ?™ê¸°??ê²½ë¡œ:"))
        ...
        # ?’¡ [?µì‹¬] iCloud ì²´í¬ë°•ìŠ¤
        icloud_auto_chk = QCheckBox("?ë™ê°ì? ë°?ì²˜ë¦¬?œì„±??iCloud")
        ...
        # ?’¡ [?µì‹¬] NAS ì²´í¬ë°•ìŠ¤
        nas_auto_chk = QCheckBox("?ë™ê°ì? ë°?ì²˜ë¦¬?œì„±??NAS")
        ...
        # ?’¡ [?¤ì—¬?°ê¸° ?„ë²½ ?˜ì •] ???¤í˜?´ìŠ¤ ?¼ìš© ë¬¸ì œë¥??´ê²°?ˆìŠµ?ˆë‹¤.
        def save_all():
            ...
            get_logger().log(f"?™ï¸ ?¤ì • ë³€ê²?- iCloud ?ë™: {icl_checked} / NAS ?ë™: {nas_checked}")
    ...
    def _open_srt_in_editor(self, srt_path: str):
        ...
        def _on_manual_save(segs):
            self._save_srt(srt_path, segs)                          # ?¤ì œ .srt ?€??
            self._backup_nums[srt_path] = self._backup_nums.get(srt_path, 1) + 1  # ë²ˆí˜¸ ì¦ê?
            self._backup_srt(srt_path, segs)                        # ??ë²ˆí˜¸ë¡?ë°±ì—…
        ...
        # ?’¡ [?˜ì • ?? ë³µì¡??ë¡œì§??ì§€?°ê³  ê¹”ë”?˜ê²Œ ì°½ì„ ?«ë„ë¡?? ë„?©ë‹ˆ??
    ...
    def _save_srt(self, srt_path: str, segments: list[dict]):
        try:
            # ?’¡ [?¬ê¸° ?˜ì •] ?›ë‚  ?´ë¦„??srt_writerë¥????´ë¦„?¼ë¡œ ë°”ê¿”ì£¼ì„¸??
            # ?˜ì • ?? from srt_writer import save_srt
            from core.subtitle_engine import save_srt
            save_srt(segments, srt_path, apply_offset=False)
            get_logger().log(f"??{os.path.basename(srt_path)} ?€???„ë£Œ")
        except Exception as e: 
            get_logger().log(f"??SRT ?€???¤íŒ¨: {e}")

    def _backup_srt(self, srt_path: str, segments: list[dict]):
        """?ë§‰ë°±ì—… ?´ë”???„ì¬ ë°±ì—… ?¬ë¡¯?¼ë¡œ ?€??(??–´?°ê¸°)."""
        try:
            from core.subtitle_engine import save_srt
            import datetime
            ...
            backup_dir = os.path.join(os.path.dirname(srt_path), "?ë§‰ë°±ì—…")
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self, "_editor_widget") and self._editor_widget and self.stack.currentIndex() == 1:
            ...
            if has_dirty:
                # [v01.00.04] editor_pipeline._on_prev/exit?€ ?™ì¼?????„ë‹ˆ??ì·¨ì†Œ ?¤ì´?¼ë¡œê·?
                msg_box = QMessageBox(self)
                msg_box.setWindowTitle("ì¢…ë£Œ ?•ì¸")
                msg_box.setText("?˜ì •???´ìš©???€?¥í•˜?œê² ?µë‹ˆê¹?")
                ...
                msg_box.button(QMessageBox.StandardButton.No).setText("?„ë‹ˆ??)
                msg_box.button(QMessageBox.StandardButton.Cancel).setText("ì·¨ì†Œ")
    ...
    def _remove_old_editor(self):
        ...
        if old:
            ...
            # 3. ?’¡ [?ˆì „ ?´ì??? ì¦‰ì‹œ ?? œ?˜ì? ?Šê³  ë³´ê??˜ì—¬ ë©”ëª¨ë¦??•ê? ë°©ì?
    ...
    def _init_editor(self, target_file: str, is_batch=False):
        ...
        # ?’¡ [?µì‹¬] ?Œì´?„ë¼?¸ì—???¸ì‹?˜ë„ë¡?is_auto_start ë³€?˜ë? ?¬ìš©?©ë‹ˆ??
        ...
        if is_batch:
            if hasattr(editor, 'update_status'):
                editor.update_status("?? AI ?”ì§„???œì‘?˜ì—ˆ?µë‹ˆ?? (?ë™ ê°ì?)")
        ...
        # ?’¡ [êµì •] pipeline.py?ì„œ ì²´í¬?˜ëŠ” ë³€?˜ëª…??is_auto_startë¡?ë³€ê²½í•©?ˆë‹¤.
        editor.is_auto_start = is_batch 
