    ...
    try:
        ...
        if get_logger:
            get_logger().log(
                ...
                f"mask={mask} cells={len(positions)}/9 threshold={threshold:.2f} "
        ...
        prev_t = 0.0
        last_emit_t = -9999.0
        cooldown_sec = max(0.75, step * 1.25)
        ...
        for frame_no in range(0, frame_count, step_frames):
            ...
            if prev_gray is not None:
                ...
                strong_cells = sum(1 for d in cell_deltas if d >= threshold * 0.70)
                enough_cells = strong_cells >= max(1, int(round(len(positions) * 0.50)))

                if delta >= threshold and enough_cells and (t - last_emit_t) >= cooldown_sec:
                    sec = round(t, 3)
                    row = {
                        "time": sec,
